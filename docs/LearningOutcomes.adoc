...
Note how components of CardBank have well-defined APIs. For example, the API of the `Logic` component is given in the link:{repoURL}/src/main/java/seedu/address/logic/Logic.java[`Logic.java`]
...
Note how the CardBank app uses Java ``assert``s to verify assumptions.
...
* Add more assertions to CardBank as you see fit.
...
Note <<DeveloperGuide#logging, how the CardBank app uses Java's `java.util.log` package to do logging>>.
...
Add more logging to CardBank as you see fit.
...
Note how CardBank uses the `ReadOnly*` interfaces to prevent objects being modified by clients who are not supposed to modify them.
...
Analyze the CardBank code/design to identify,
...
Note <<UsingGradle#, how the CardBank app uses Gradle to automate build tasks>>.
...
Note <<UsingTravis#, how the CardBank app uses Travis to perform Continuous Integration>>. (https://travis-ci.org/se-edu/addressbook-level4[image:https://travis-ci.org/se-edu/addressbook-level4.svg?branch=master[Build Status]])
...
* Test method `handleCardBankChangedEvent_exceptionThrown_eventRaised()` is a unit test because it uses _dependency injection_ to isolate the SUT `StorageManager#handleCardBankChangedEvent(...)` from its dependencies.
