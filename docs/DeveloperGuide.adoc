...
* at app launch: initializing all components in the correct sequence and connecting them up with each other.
...
.Relevant Components interactions for `delete 1` command (part 1)
image::SDforDeleteActivity.png[width="800"]

[NOTE]
Note how the `Model` simply raises a `DeskBoardChangedEvent` when the Desk Board data
...
.Relevant Components interactions for `delete 1` command (part 2)
...
image::TaskCard.png[width="650"]

.EventListCard example
image::EventCard.png[width='650']
...
.Relevant components interaction for `list task` command (part 1)
...
.Relevant components interaction for `list task` command (part 2)
...
Similar to how `Task` and `Event` objects inherit from `Activity` class (as discussed in <<Implementation - Activity, Managing Activities>>),
...
=== Auto overdue
The auto overdue feature will automatically tag tasks as "overdue" once their due date times have passed and tag events as "finished" once their end date times have passed.
The current implementation and design considerations for this feature are explained in this section.

==== Current Implementation
The auto overdue feature is implemented using the `ScheduledExecutorService` class, provided by the `java.util.concurrent` package. This class creates another thread in the system and that thread is scheduled to run a checker every 30 seconds.

It is implemented in `DateTimeScheduler` as shown:
[source, java]
----
...
The `checker` is implemented as shown:
...
==== Design Considerations
This section describes various design considerations related to the auto overdue feature.

* Alternative 1 (current choice): Using `ScheduledExecutorService` to check for overdue tasks and finished events every 30 seconds.
...
* Alternative 2: Using `ScheduledExecutorService` to check for overdue tasks and finished events every 1 hour.
...
* **keeping track** of overdue tasks and finished events with the auto overdue feature and `overdue` command.
