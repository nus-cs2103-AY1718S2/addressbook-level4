...
//tag::Login/Logout[]
=== Lock/unlock & reset password feature
...
==== Current Implementation
image::SwitchSequenceDiagram.png[width="400"]
...
See sequence diagram. The archival feature is implemented by triggering an `SwitchThemeRequestEvent()` which is then handled in `MainWindow`.
The theme switching in done in two ways. For the main window, it is done by changing the CSS stylesheet used. For the calendar, because the stylesheet is complicated and is provided by an external library, we use a JavaFX effect to invert all colors.
...
The edit appointment feature is interesting in that editing persons check whether the person is already in the appointment. If so, the person is removed. Otherwise, the person is added.
This differs from the behaviour of editing tags of a person, which is destructive and overwrite all tags.
...
=== Email feature
==== Current Implementation
The email feature allows the user to send emails in the app by typing their message in an email compose pop up box.
The inbox feature allows the user to see a preview of the plain text emails that they have received right when they open the app. To view the message, the user clicks the message and an popup will appear with the full message.
...
|`* * *` |user |Login/logout to my Addressbook to modify my contacts and appointments |protect my data

|`* * *` |user |schedule meetings with my clients |remember when I am supposed to meet them

|`* * *` |user |conveniently send emails to clients |increase efficiency and save time

|`* * *` |user |conveniently see my email inbox |know when my clients are contacting me
...
|`* *` |user |export my contacts to a file |export the data to somewhere else if I need it
...
//tag::OngKuanYangTestCases[]
=== Add appointment

. Adding an appointment

.. Test case: Run `addappointment` with valid parameters. +
   Expected: Appointment added.
.. Test case: Run `addappointment` with invalid parameters. +
...
.. Test case: Run `deleteappointment 1` +
   Expected: The first appointment is deleted
.. Test case: Run `deleteappointment InvalidIndex` +
...
.. Test case: Run `editappointment` with valid parameters. +
   Expected: Appointment edited.
.. Test case: Run `editappointment` without any edit information. +
   Expected: Error message displayed.
...
.. Test case: Run `archive INDEX` where INDEX is a valid person index. +
   Expected: Person disappears from list.

=== List all clients

. Listing all clients including archived

.. Prerequisite: Archive some client as in the archive client test.
.. Test case: Run `listall` . +
...
.. Prerequisite: Some person archived and listed as in List all clients test.
.. Test case: Run `unarchive INDEX` where INDEX is the index of an archived person. +
...
.. Test case: Run `switch` . +
   Expected: Theme is switched from dark to light and vice-versa.
.. Test case: Close and open the application. +
   Expected: Theme is saved across restarts.
// end::OngKuanYangTestCases[]

//tag::XavierMaYuqianTestCases[]
=== Lock

. Lock the addressbook to prevent further modification on contacts or appointment
...
. Unlock the addressbook for further modification on contacts or appointment

.. Prerequisite: The addressbook is already locked.
.. Test case: Run `unlock PASSWORD` . +
...
.. Test case: Run `setPassword OLD_PASSWORD NEW_PASSWORD` . +
...
=== Remove tags from all contacts

. Removing tags from all contatcs

.. Prerequisite: There are already some tags in the addressbook.
.. Test case: Run `removetag TAGNAME` . +
...
.. Prerequisite: There are already some contactss in the addressbook.
.. Test case: Run `export` . +
...
.. Prerequisite: There are already some contacts in the addressbook.
.. Test case: Run `add n/David p/98765432 e/johnd@example.com a/311, Clementi Ave 2, #02-25 tz/PST c/interested in football t/friends t/owesMoney` . +
   Expected: A contact is created with comment interested in football.
.. Test case: Run `edit 1 c/interested in basketball` . +
   Expected: The first contact's comment is modified to interested in basketball.
//end::XavierMaYuqianTestCases[]

//tag::GloriaLawTestCases[]
=== Incorrect time zones are rejected

. Time zone abbreviations unknown to Java are rejected

.. Prerequisite: There is at least one contact in Sell IT
.. Test case: Run `add n/David p/98765432 e/johnd@example.com a/311, Clementi Ave 2, #02-25 tz/PST c/interested in football t/friends` . +
   Expected: A contact is created with the time zone PST is created.
.. Test case: Run `edit 1 tz/PST` .+
   Expected: The client with index 1 has their time zone changed to PST
.. Test case: Run `add n/David p/98765432 e/johnd@example.com a/311, Clementi Ave 2, #02-25 tz/ c/interested in football t/friends` . +
   Expected: Error message: `Time zones should be have the standard time zone abbreviations, and should not be blank`.
.. Test case: Run `edit 1 tz/P$T` . +
   Expected: Error message: `Time zones should be have the standard time zone abbreviations, and should not be blank`.
...
.. Prerequisite: There is at least one email in Sell IT
.. Test case: Click a client's email in the person panel and enter a subject and the email message body and click the send button. +
   Expected: Email sent successfully window.
.. Test case: Click the new email button after clicking an email in the inbox and do not enter any recipients and click the send button. +
   Expected: Could not send email due to an error window.
.. Test case: Click the new email button after clicking an email in the inbox and enter `abcde` as the recipient and click the send button. +
   Expected: Could not send email due to an error window.
.. Test case: Click the new email button after clicking an email in the inbox and enter `@gmail.com` as the recipient and click the send button. +
   Expected: Could not send email due to an error window.

=== Reply to an client email

. Reply to an email an email received from a client

.. Prerequisite: There is at least one email in Sell IT.
.. Test case: Click the reply button after clicking an email in the inbox and type some text in the email body. +
   Expected: Email sent successfully window.
.. Test case: Click the reply button after clicking an email in the inbox and add an invalid recipient such as `asdf`. +
   Expected: Could not send email due to an error window.

=== Forward a client email

. Forward an email received from a client

.. Prerequisite: There is at least one contact or email in Sell IT
.. Test case: Click the forward button after clicking an email in the inbox and enter a valid email and click send +
   Expected: Email sent successfully window.
.. Test case: Click the forward button after clicking an email in the inbox and enter an invalid recipient such as `asdf`. +
   Expected: Could not send email due to an error window.

=== Load all client emails.

. Load all client emails.

.. Prerequisite: There is at least one email message received in Sell IT
.. Test case: See which emails are loaded onto the email panel. On a browser, log onto the gmail with the email "sg.salesperson@gmail.com" and password "gloriacs2103" and verify they are the same. +
   Expected: Email inbox should be the same as what is shown on gmail.

=== Receive a client email

. Receive an email from a client.

.. Prerequisite: None
.. Test case: From an email you have access to, send an plain text email to "sg.salesperson@gmail.com". +
   Expected: Email appears on the email panel within 30 seconds and the full view is opened in a pop up when clicked.
//end::GloriaLawTestCases[]
