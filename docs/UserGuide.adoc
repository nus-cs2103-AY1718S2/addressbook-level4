...
.  Ensure you have Java Runtime Environment (<<JRE>>) 1.8.0_60 or later installed in your Computer. You can download from http://www.oracle.com/technetwork/java/javase/downloads/jre8-downloads-2133155.html[here].
[NOTE]
...
[NOTE]
If you counter any problems while using ReciRecipé, you might want to refer to <<faq>> to see common issues and the solutions.
...
{nbsp} +
Select "Open command window here" (earlier versions of Windows) or "Open PowerShell window here" (Windows 10), type in:
[source]
----
...
| *Ingredient search* |  `ingredient KEYWORD [MORE_KEYWORDS]...`
                    |   E.g. *`ingredient chicken rice`*
| *Internet search* |  `search RECIPE_NAME`
                    |   E.g. *`search hainanese chicken rice`*
| *List*            |  `list`
                    |  *`list`*
| *Parse*           |  `parse`
                    |  *`parse`*
