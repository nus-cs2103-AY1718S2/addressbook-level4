...
=== NUS Commands
==== Viewing common time slots in timetable: `union` (coming in v2.0)

Displays the common free time of multiple people in StardyTogether.(Minimum: 2) +
Format: `free ODD/EVEN INDEX INDEX [INDEX]...`

****
* Only indexes of the current filtered list are valid
* Overlap of the persons' timetables will be displayed.
****
...
* Uploads an instance of StardyTogether to the specified `FILENAME`.
* Encrypt that instance using the `PASSWORD` provided.
* If no `PASSWORD` is provided, the instance will be exported without encryption.
* The filepath refers to the filepath where the instance file will be stored.
****
...
* `upload data/addressBook.xml testpassword` +
