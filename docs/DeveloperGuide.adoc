...
===== Aspect: Support for multiple themes
...
* `learningPhase` the number of days we denote as a learning phase.
* `lastInterval` the last interval given to schedule a review.
* `easingFactor` how easy is it for the user to remember a fact,
...
Any feedback within this period will not update the `easingFactor`.

`feedback()` will log down the number of `success` and `failures` so far.
