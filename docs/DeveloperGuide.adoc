...
You may wish to refer to the <<Setting up, setting up>> portion
...
Upon launching the app, the Login component takes in two inputs from the user: `Username` and `Password`, then stores the user's login credentials into a `.xml` file.
...
As seen from the layout in figure 7, from top to bottom: +

* MenuBar, `File` and `Help` are dropdown menus that perform their usual functions. As seen in Figure 8 below.

.Menu bar of the UI
image::MenuBar.png[width="800"]

* To the left of the application (ass seen in Figure 9 below) is a list that consists of any `Person` entries in the address book.
...
* To the right, is the calendar of the address book, in v2.0 tasks can be scheduled into the calendar. As seen in Figure 11 below.
