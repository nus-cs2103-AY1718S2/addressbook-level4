...
Want to find a specific book? Browsing for new books? Use the `search` command. +
...
CAUTION: You can only perform `search` with internet connection.
...
CAUTION: You can only perform `add` when the list shown is a search result.
...
CAUTION: You can only perform `edit` when the list shown is from your book shelf.
...
CAUTION: You can only perform `delete` if the list shown is from your book shelf.

TIP: You don't have to delete books after reading them. Simply mark them as read. They may serve as useful reference in the future.
