...
By: `F09-B1`      Since: `Mar 2018`      Licence: `MIT`
...
image::UpdateDisplayCommandSequenceDiagram.png[width="700"]
...
Upon every launch of the application, during the initialization of the `ModelManager` class, it will run through the `UniqueItemList` and delete
...
The fileanme of the image copied over will be a SHA-256 hash. This hash is calculated over the `Person` object's details (i.e Name, Phone and Email) to ensure a unique filename.
The following activity diagram in Figure 16 shows how the files are saved:

.Activity diagram for storing image files
image::StoreDisplayPictureActivityDiagram.png[width="700"]
