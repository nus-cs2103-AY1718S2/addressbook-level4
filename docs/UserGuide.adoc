...
= image:CodeducatorTitle.png[Codeducator,100,role="le"] Codeducator - User Guide
...
. *Format of command* +
  The defined structure of the command. See <<Command Format>> for more information
. *Steps Taken* +
...
+
.Type `login` into the command box
...
+
.Google's OAuth/authentication window
...
+
.Google's login success message
...
+
.Codeducator's login success message
...
. Authorising Codeducator is important! Denying access will stop Codeducator for accessing your account.
. If you have already logged in before without logging out, you may skip the login process by just typing in `login`
. If you do not successfully login , do not fret! Just repeat the steps to login again.
====
...
+
.Viewing your contacts list in link:{calendarURL}[calendar.google.com]
image::syncResult1.png[width="790"]
+
.Viewing your schedule in link:{contactsURL}[contacts.google.com]
image::syncResult2.png[width="790"]
+
. Congrats, your data has been synchronised! You are free to use them across your devices
