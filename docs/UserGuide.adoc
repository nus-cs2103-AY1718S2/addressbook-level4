...
* Words in upper case are the parameters to be supplied by the user e.g. in `add name/NAME`, `NAME` is a parameter which can be used as *`add name/Chicken rice`*.
...
** `CALORIES` and `SERVINGS` are positive numbers. E.g. `120`, `5000`, etc.
...
See figures below for the expected behavior of this command. +
{nbsp}
...
* *Add*: `add name/RECIPE_NAME [ingredient/INGREDIENT] [instruction/INSTRUCTION] [cooking_time/COOKING_TIME] [preparation_time/PREPARATION_TIME] [calories/CALORIES] [servings/SERVINGS] [url/LINK] [tag/TAG]...` +
E.g. *`add name/Chicken Rice ingredient/chicken, rice cooking_time/10m tag/good tag/fast`*
* *Clear* : `clear`
* *Delete* : `delete INDEX` +
E.g. *`delete 3`*
* *Edit* : `edit INDEX [name/RECIPE_NAME] [ingredient/INGREDIENT] [instruction/INSTRUCTION] [cooking_time/COOKING_TIME] [preparation_time/PREPARATION_TIME] [calories/CALORIES] [servings/SERVINGS] [url/LINK] [tag/TAG]` +
