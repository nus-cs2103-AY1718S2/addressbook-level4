...
public class DeleteCommandSystemTest extends IMDBSystemTest {
     ...
     */
    private Patient removePerson(Model model, Index index) {
        Patient targetPatient = getPerson(model, index);
        try {
            model.deletePerson(targetPatient);
        } catch (PatientNotFoundException pnfe) {
            throw new AssertionError("targetPatient is retrieved from model.");
        }
    ...
    private void assertCommandSuccess(Index toDelete) {
        Model expectedModel = getModel();
        Patient deletedPatient = removePerson(expectedModel, toDelete);
        String expectedResultMessage = String.format(MESSAGE_DELETE_PERSON_SUCCESS, deletedPatient);
