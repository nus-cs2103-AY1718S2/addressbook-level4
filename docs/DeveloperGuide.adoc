...
If the input is indeed a URL that links to an image, `Image` will go through several steps to download the image and set the new image path (<<fig-DownloadImageAD>>).

[[fig-DownloadImageAD]]
.Activity diagram of an image downloading process
image::DownloadImageAD.png[width="400"]
{nbsp} +
...
For the detailed implementation of each step, see link:{repoURL}/src/main/java/seedu/recipe/model/recipe/Image.java[`Image.java`].
