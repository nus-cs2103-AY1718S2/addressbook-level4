...
* *`shadowJar`* +
Creates the `progresschecker.jar` file in the `build/jar` folder, _if the current file is outdated_. +
...
Runs all tests.
* **`guiTests`** +
Runs all tests in the `seedu.progresschecker.ui` and `systemtests` package
* **`nonGuiTests`** +
Runs all non-GUI tests in the `seedu.progresschecker`
package
* **`headless`** +
