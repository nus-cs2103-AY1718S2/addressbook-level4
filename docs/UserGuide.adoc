...
***
=== Assigning a Customer to a Runner : `assign` [since v1.4]

[NOTE]
====
* This command assigns a `Customer` to a `Runner` using specified `INDICES`.
** The `INDEX` refers to the index number shown in the last person listing.
** The `INDEX` must be a *positive integer*: 1, 2, 3, ...
====
...
=== Assigning multiple `Customers` to a `Runner`
