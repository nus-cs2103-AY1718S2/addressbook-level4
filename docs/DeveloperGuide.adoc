...
[[SettingUp]]
== Setting up
Follow the instructions below to set up the project in your computer.

=== Prerequisites

.  Install *JDK `1.8.0_60`* or later
+
[NOTE]
...
written using https://github.com/google/guava/wiki/EventBusExplained[Google's Event Bus library])
* `LogsCenter` : Used by many classes to write log messages to the App's log file.

The rest of the App consists of four components.

* <<Design-Ui,*`UI`*>>: The UI of the App.
* <<Design-Logic,*`Logic`*>>: The Command Executor of the App.
* <<Design-Model,*`Model`*>>: Data Holder of the App in-memory.
* <<Design-Storage,*`Storage`*>>: Data Reader and Writer to hard disk.
...
* can save the Desk Board data in xml format and read it back.
...
Since we need now to store information of event and task in the memory now.
The `Person` Class need to modified to store the record..

The `UniquePersonList` need to be modified to store list of `Task` and `Activity`.
{empty} +
{empty} +
