...
.  Double-click the file to start the app. The Login GUI should appear in a few seconds.
.  Enter your username and password. Press Enter.
.  If login is successful, the GUI should appear.
+
...
=== Opening a calendar : `calendar`

Opens a calendar in a webview. Will eventually have the option to be opened in the in application scene switcher.
Format: `calendar`
...
=== Adding and event to the calendar : `event-add`

Allows a user to add an event directly to their calendar.
Format: `event-add t/<title> l/<location> ds/<start date/time> de/<end date/time> n/<notes>`

=== Deleting an event from the calendar : `event-delete`

Displays a list of upcoming events to the user and lets them select one to delete.
Format: `event-delete`

=== Editing an event in the calendar : `event-edit`
...
SLAP will request for your login credentials (username and password) before allowing you to access it.

=== Logout from your account: `logout`

SLAP allows to log out from your account so that you can ensure confidentiality of your data.
