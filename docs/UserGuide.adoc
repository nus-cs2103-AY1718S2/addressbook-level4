...
The following image illustrates the command format used in Medeina. 

.Command format
image::commandSyntax.png[width="500"]
...
* They help to reduce typing.

.^a|Options
| `**-o**`, `**-p**`, `**-a**`
a| * Options are used in `add`, `edit`, `find`, `delete` and `listappt` commands.
* They are specified by the user to execute variants of a command. For example in figure 2, `**-p**` option denotes editing a pet patient.
* If there are multiple options used in a command, order must be preserved.
...
* Please refer to figure 1 to find out where you can see indexes.

.^a|Prefixes
| `n/`, `p/`, `nr/`, `e/`, `s/`, `t/`
a| * Prefixes mark the start of a parameter. 
...
a| * Values to be supplied by the user
* Parameters are represented by words in upper case in this guide and in error messages. 
...
* Suggestions for species, breed, colour and blood type will be shown when their corresponding prefixes is detected.
