...
* **Alternative 1 (current choice)**: Let UI component handle the parsing of UniquePersonList obtained from Event
...
* **Alternative 2 (current choice)**: Let Parser class handle the parsing of UniquePersonList obtained from Event
** Pros: More modularity.
** Cons: Not apparent in usage by User. Functionality remains the same but coupling between UI and Parser increases.

===== Aspect: How User can open Birthday List

* **Alternative 1 (current choice)**: Manual command "birthdays"
** Pros: Easy to implement seperately as a command.
** Cons: Not very user-friendly.
* ** Alternative 2:** Notification at the start of app if a birthday is occurring today
** Pros: Additional useful functionality for User.
...
* ** Alternative 1 (Current choice)**: Fixed format as DDMMYY
** Pros: Less room for errors.
** Cons: User may not like the DDMMYY format.
...
|`* * *` |busy student |have my customised short forms of commands |type even quicker and in my own style
