...
[TIP]
Use the -f flag to view all person in favourites

Examples:

* `list -f` +
List only all person that you added to favourites
* `list` +
List all persons
...
=== Adding a person to favourites : `fav`

Adds an existing person in the address book to your favourites. +
Format: `fav INDEX`

****
* Add the person at the specified `INDEX` to your favourites.
...
* `list` +
`fav 2` +
Adds the 2nd person in the address book to favourites.
* `find Betsey` +
`fav 1` +
Adds the 1st person in the results of the `find` command to favourites

=== Remove a person from favourites : `unfav`

Removes an existing person in the address book from your favourites. +
Format: `unfav INDEX`

****
* Removes the person at specified `INDEX` from your favourites.
...
* `list` +
`unfav 2` +
Removes the 2nd person in the address book from favourites.
* `find Betsey` +
`unfav 1` +
Removes the 1st person in the results of the `find` command from favourites
...
Finds persons whose labels contain any of the given keywords. +
...
When your AddressBook has a person named John Doe, which you have tagged t/friends and t/owesMoney, and a person named Betsy which you have tagged t/owesMoney and t/poor,
* `findTag friends` +
Returns `John Doe`
* `findTag friends owesMoney` +
Returns any person having tags `friends`, `owesMoney`, i.e. `John Doe` and `Betsy`
...
Selects the person identified by the index number used in the last person listing, and display their address on google maps. +
Format: `select INDEX`

****
* Selects the person at the specified `INDEX` and loads their location on Google Maps.
* The index refers to the index number shown in the most recent listing.
* The index *must be a positive integer* `1, 2, 3, ...`
****
