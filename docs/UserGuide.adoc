...
Adds a recipe to the instruction book +
...
Shows a list of all recipes in the instruction book. +
Format: `list`

=== Editing a recipe : `edit`

Edits an existing recipe in the instruction book. +
...
Edits the phone number and ingredient instruction of the 1st recipe to be `91234567` and `johndoe@example.com` respectively.
...
Deletes the specified recipe from the instruction book. +
Format: `delete INDEX`
...
* `list` +
`delete 2` +
Deletes the 2nd recipe in the instruction book.
* `find Betsy` +
`delete 1` +
...
* `list` +
`select 2` +
Selects the 2nd recipe in the instruction book.
* `find Betsy` +
`select 1` +
...
Restores the instruction book to the state before the previous _undoable_ command was executed. +
Format: `undo`

[NOTE]
====
Undoable commands: those commands that modify the instruction book's content (`add`, `delete`, `edit` and `clear`).
====
...
Clears all entries from the instruction book. +
Format: `clear`
