...
The event is then propagated through the `_EventsCenter_` to the `*UI*`. The `*UI*` will call `handleInternetSearchRequestEvent()` to update the theme accordingly. Refer to <<fig-InternetSearchEventSD>>.
