...
// tag::calendar[]
=== Implementation of Calendars and Operations on Calendars
==== Current Implementation

To implement the calendars for employees, we use a third party source which is Google Calendar API, which allows to manage google
...
on the main calendar. After that, it will post an event to request to open a todo-list window. And post another event containing all the retrieved events.
...
Then they will just sent request through the API service and all other things will be done at google side. Finally, they will do updates according to the request results.

==== Design Considerations
===== Aspect: Where to place the todo-list window
* **Alternative 1 (current choice):** Make it as a pop-up window.
** Pros: Easy to manage, as it is created at a new javaFX stage.
...
** Pros: Every time execute `todoList` command, the information can be updated.
...
running, and unlock it to resume at anytime.

The password is stored as a field annotated as `@XmlElement` in `XmlSerializableAddressBook`, and is been read and converted to model type when the application
is launched and saved at `LogicManager` in which the password can be handled at runtime.

Below is the overall design of Lock and Unlock system, lines in blue, red and green are for event posting and handling.
...
. `LockCommandParser` is to make sure no other extra inputs are entered besides the command word `lock`. Refer to the first code snippet below.
...
no information should be shown now. Note the `DetailPanel` is the handler of the event. There are mainly two parts that need to be hidden: `PersonList` and `DetailPanel`. Refer to the second code snippet below.
. `LogicManager` is used to check the state and decide whether the application is locked or not, and decide the logic flow accordingly.

Below is how `LockCommandParser` deals with user input:
[source,java]
----
...
. `UnlockCommand` is used to post an event requesting `MainWindow` to pop up a password input dialog, get the password entered by user, and
compare it with the application's unlock password to decide whether the unlock process is successful. Refer to the code snippet below.
...
[NOTE]
This is a simplified sequence diagram, the detail of the instantiation of `UnlockCommand` is removed, because it is similar to that of
...
** Pros: Simpler implementation than current one, no need extra effort for controlling.
...
Also need to keep the encryption key secret.
