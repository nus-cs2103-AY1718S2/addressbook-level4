...
* Shorthand equivalent alias, or better known as short-form, is available for all commands to simplify execution of commands e.g. `a` is the alias for `add`.
...
.Medeina's autocomplete
image::autocomplete.png[width="700"]

* Medeina supports autocomplete for command words, options, prefixes, and parameters.
...
However, user should take note not to modify the default files in anyway. Manipulating the data inside files such as Medeina.xml, might cause exceptions (the application may still run, but will not be able to return error messages).
...
*A*: The issue is embedded inside CalendarFx API, it has already been documented in our developer guide as well. Hopefully the issue can be resolved in future releases of CalendarFX.

*Q*: When right click on the calendar day view, a window jump out and says "unable to create a new entry", why is that? +
*A*: Since Medeina is a command line based application, clicking on calendar to create appointments have been disabled. User should create appointments using `add` command.
