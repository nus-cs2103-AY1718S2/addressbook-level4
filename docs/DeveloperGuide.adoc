...
Currently, a class called `SuggestionsPopUp`, which inherits from `ContextMenu`, is created to handle all operations relating to auto-completion. The operations include displaying a pop-up window consisting of suggestions for user’s input, auto-completing input field when a word is chosen and hiding the window.

Key combination kbd:[Shift]+kbd:[Space] is used to show suggestion pop-up and for other keys and key combinations, the pop-up is hidden. The code to handle these key and key combination actions is shown as below:
...
The given list and the constant `MAX_SUGGESTIONS` can be found in `AutoCompletionUtil.java`. All operations with input text is supported by `TextInputProcessorUtil.java`.

To calculate position to display pop-up menu, height and width of input text (in pixels) are computed and then used to determine display position in the main window. Height and with of input text is calculated in `TextInputProcessorUtil.java`.

The suggestion list is then shown in main window.

To hide suggestion list, a method called `hide()` is simply implemented as below:
...
To automatically filling text field with chosen word, an `EventListener` is attached to each item in the pop-up menu. When an item is chosen, it replaces the last word in input field with the word associated with the item. The code is shown below:
