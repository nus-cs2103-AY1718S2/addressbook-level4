...
It fully supports the `undo` and `redo` commands as well.
...
[NOTE]
When the `DisplayPic` object is first created, it stores the _original_ filepath of the image file provided by the user. This will later be updated by the 3 commands to the duplicated image filepath.
...
===== Fetching the image file to display
