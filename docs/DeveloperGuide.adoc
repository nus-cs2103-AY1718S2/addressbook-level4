...
* A Student object called "editedStudent" will be created by `FavouriteCommand.createEditedStudent(Student)`, which is implemented as such:
[source, java]
----
...
* `Model.updateStudent(Student, Student)` is used to replace studentToFavourite with editedStudent in the Address Book in-memory
