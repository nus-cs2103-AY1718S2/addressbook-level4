...
image::ModelComponentDashboardClassDiagram.png[width="800]
...
image::StudentClassDiagram.png[width="800"]

Since the implementation of the favourite and unfavourite command are similar, we will talk about the implementation of the favourite command only.

The logic part of the favourite command will be executed by the `FavouriteCommand` class.
To mark a `Student` object called "studentToFavourite" as favourite:

* A Student object called "editedStudent" will be returned by `FavouriteCommand.createEditedStudent(Student)` to the `FavouriteCommand.preprocess()` method.
`FavouriteCommand.createEditedStudent(Student)` is implemented as such:
...
----
    private static Student createEditedStudent(Student target) {
        ...
        // ... get the rest of the target's attributes ...
...
* editedStudent will have the attributes of studentToFavourite, except that its `Favourite` attribute will be set to "true"

In the `FavouriteCommand.execute()` method, `Model.updateStudent(Student, Student)` is called to replace studentToFavourite with editedStudent in the Address Book in-memory.

[NOTE]
If you have a future implementation that requires the addition of a new attribute in the `Student` class, you must take note of updating the `FavouriteCommand.createEditedStudent(Student)` method to reflect the new attribute.
...
image::ModelWithPLFeature.png[width="800"]
...
`Dashboard` and `Milestone` are exposed to `ObservableList<Milestone>` and `ObservableList<Task>` respectively that can be 'observed'.
This means that the UI can be bound to both of the lists so that the UI automatically updates when the data in both lists change.
...
For example, in the `AddMilestoneCommand`, to add a new milestone object to the dashboard of a `Student` Object called "studentToEdit":

* A `Student` object called "editedStudent" will be returned by the `AddMilestoneCommand.createEditedStudent(Student, Milestone)` method to the `AddMilestoneCommand.preprocess()` method.
`AddMilestoneCommand.createEditedStudent(Student, Milestone)` is implemented as such:
[source, java]
----
    private Student createEditedStudent(Student studentToEdit, Milestone newMilestone)
        ...
        // ... get remaining attributes of the student ...
...
----
* The new `Milestone` object is added to the `UniqueMilestoneList` object that belongs to the `Dashboard` of "studentToEdit"
* A new `Dashboard` object is created with the updated `UniqueMilestoneList` object
* "editedStudent" is created with the same attributes of "targetStudent", but with the new `Dashboard` object

In the `AddMilestoneCommand.execute()` method, `Model.updateStudent(Student, Student)` is called to replace "targetStudent" with "editedStudent" in the Address Book in-memory

[NOTE]
If you have a future implementation that requires the addition of a new attribute in the `Student` class, you must take note of updating the `X.createEditedStudent(...)` method, where X is any command that modifies the dashboard, to reflect the new attribute.
...
* *(Major)* Create a "more information" page storing other important details of the students including an optional profile picture.
...
E.g. emergency contact and an optional profile picture
...
|`* * *` |user | be able to edit the miscalleneous information of a student |have the most updated version of a student's information
