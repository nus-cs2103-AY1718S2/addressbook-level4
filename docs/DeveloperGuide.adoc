...
The autocomplete mechanism is facilitated by `LogicManager`, which exposes `COMMAND_LIST` from `CommandList` for `CommandBox` to use. It supports the predicting of a word or phrase that the user may type based on a partial text query.
...
image::AutocompleteBefore.png[width="100"]
...
image::AutocompleteNav.png[width="100"]
