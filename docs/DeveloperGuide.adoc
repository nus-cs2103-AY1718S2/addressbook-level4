...
.. Test case: `list` then `export VALID_FILE_PATH` +
   Expected: All students, tags, and alias from StardyTogether are exported to filepath.
.. Test case: `find alex` then `export INVALID_FILE_PATH` +
   Expected: All students with `Alex` in his/her name, tags, and aliases from StardyTogether are exported to filepath.
.. Test case: `find nonExistentName` then `export INVALID_FILE_PATH` +
   Expected: All tags, and aliases from StardyTogether are exported to filepath. No students are exported.
.. Test case: `export INVALID_FILE_PATH` +
   Expected: error message will be shown
.. Other incorrect export commands to try: `export`, `export filepath  wrongPassword  ` (notice the spaces) +
   Expected: Similar to previous.
...
.. Test case: `import VALID_FILE_PATH` +
   Expected: All students, tags, and alias from the imported file are added to StardyTogether.
.. Test case: `import INVALID_FILE_PATH` +
   Expected: error message will be shown
.. Other incorrect delete commands to try: `import`, `import filepath wrongPassword` (where wrongPassword is wrong), `import INVALID_FILE_FORMAT` (where file is in invalid format) +
   Expected: Similar to previous.
