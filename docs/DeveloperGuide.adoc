...
IntelliJ should look like the figure below after step 3.
...
<<Design-Logic,*`Logic`*>> contains all commands and this is how <<Design-Logic,*`Logic`*>> reacts when the user submit a command:
...
Refer to diagram below <<fig-LogicClassDiagram>>.
...
** Pros: NA.
** Cons: Need additional works to make `_JPopupMenu_` compatible with `_TextArea_`.
...
Refer to <<fig-ChangeThemeDetailsSD>>
