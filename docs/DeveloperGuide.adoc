...
The UI consists of a `MainWindow` that is made up of parts e.g.`CommandBox`, `ResultDisplay`, `PersonListPanel`, `StatusBarFooter`, `CreditorPanel`, `DebtorPanel`, `TransactionPanel` etc. All these, including the `MainWindow`, inherit from the abstract `UiPart` class.
...
DebtsTable is a table that collects the debts between every person.

This feature simplified the calculation, the storing and the retrieving of every debts between the persons added.
The `DebtTable` is updated when a person or a transaction is added, edited or removed. +

* Implementation of the DebtsTable:

** **Alternative 1 (current choice):** Using a dictionary of every person's `DebtsList`,
which are dictionaries with all persons that have debts with its owner.
*** Pros: More flexible. Easy to add and remove a person or a debt. Easy to retrieve the debt between two persons.
Will use less memory.
*** Cons: Harder to edit
** **Alternative 2:** Using an two dimensional array.
*** Pros: Easy to add or edit a person or a debt.
*** Cons: Hard to remove a person.
...
//tag::Debtors_Creditor_Panel[]
=== Debtors & Creditors List Table Mechanism

==== Debtors & Creditors List Table Implementation

Debtors and Creditor list will only be updated when a person in the person card panel list is selected. When a person is selected, PersonPanelSelectionChangedEvent will be handled by the UI and post an updateDebtorsAndCreditorList event which will be handled by logic as shown by the code below.

* Code Listing

image::PersonSelection.png[width="800"]

image::Debtors_Creditor.png[width="800"]


After which to in order to set the new creditors or debtors lists, a debtList with respect to the person selected will be retrieved using debtTable.get(person) where the person is the key in debtTable object and debtList is the value. Subsequently, using the debtList of the person selected, setDebtor(debtList) and setCreditor(debtList) method will be executed, and the FilteredDebtorsLists and FilteredCreditorList will be updated as shown by the sequence diagram above. Please refer to the diagram and code below.

* Code Listing

image::Debtors_CreditorCode.png[width="800"]

image::Debtors_CreditorCodeSet.png[width="800"]

image::Debtors_CreditorCodeSetCreditor.png[width="800"]
...
** Alternative 1: The creditor list and debtor list is not unique to each person
** Pro: The current implementation allow the creditor list and debtor list to be updated responsively based on any changes on the debtTable.
** Con: Once a creditor list or debtor list is updated, it no longer able to retrieve past data.

** Alternative 2: Each person has a unique debtors and creditor list
** Pro:  Easy retrieve of debtors and creditors
** Con: Once a new transaction is added, all the creditors and debtors list relevant to the payer or payee(s) in the transaction have to be updated
//end::Debtors_Creditor_Panel[]
...
* Major feature: Added Payment & Paydebt add Transaction command. Payment transaction command is used when a person make a payment to one or multiple people. Paydebt Transaction is used when the user return an amount owed to another person.
* Major feature: Added Debtors & Creditors panel for the user to see who owed them money or people who they owed money to
* Minor feature: Prevent person from deleted if his debt not clear.
* Minor feature: Prevent transaction from deleted if payer or payee in the transaction do not exist anymore.
...
=== Use case: Add all persons involved in the trip
