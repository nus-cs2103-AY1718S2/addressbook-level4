...
[width="100%", cols="2, 2,4 ,2 ,4 "options="header",]
|=======================================================================
| Aspect | Alternative 1 (Current choice) | Pros (+)/ Cons(-) | Alternative 2 | Pros(+) / Cons(-)
|*How to store `ProgrammingLanguage` for a `Student`*
| Store as an attribute of Student. | + Easy to keep track of as well as modify.  +
{empty} +
...
| Implement it through the existing `add` command.
...
| Have it as a JavaScript function in the HTML file of the student's profile page. | + Able to directly read the XML data of students from the file.  +
{empty} +
...
| Copy the picture file into the local jar directory.
...
|`* * *` |tutor |search a student by label |
...
|`* *` |tutor |edit a lesson for a student |keep track of timing changes

|`* * *` |tutor with a filled timetable |check free slot |easily find a free timeslot to allocate to students

|`* * *` |tutor with a filled timetable |sync with cloud storage calenders |easily sync my timetable to my schedule

|`* * *` |tutor |get an overview of my student's progress |so that I can see what they don't know and tutor them better
...
|`* *` |tutor |view hours spent teaching |see how much money I should collect
...
.. Prerequisites: List all students using the `list` command. Multiple students in the list. Your current schedule would preferably have no lessons starting before 7am or ending after 10pm.
.. Test case: `addLesson 1 d/mon st/10:00 et/12:00` +
   Expected: a lesson will be added for student at index 1, starting at 10:00am, ending at 12:00pm
.. Test case (Maximum range): `addLesson 1 d/wed st/00:00 et/23:59` +
   Expected: a lesson will be added for student at index 1, starting at 00:00pm, ending at 23:59pm (whole day).
   The schedule should resize to show the lesson in the whole day.
