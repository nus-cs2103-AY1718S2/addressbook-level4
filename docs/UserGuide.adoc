...
[TIP]
You can use kbd:[Ctrl]+kbd:[F] to search in the user guide.
...
* Words in upper case are the parameters to be supplied by the user e.g. in `add name/NAME`, *_NAME_* is a parameter which can be used as *`add name/Chicken rice`*.
...
** *_LINK_* takes a valid website address as its value. E.g. `http://google.com`.
...
Format: `add name/RECIPE_NAME [ingredient/INGREDIENT] [instruction/INSTRUCTION] [cooking_time/COOKING_TIME] [preparation_time/PREPARATION_TIME] [calories/CALORIES] [servings/SERVINGS] [link/LINK] [tag/TAG]...`
...
{nbsp} +
If you follow correctly, the result box will tell you so and your recipe will be added to the book (<<fig-AddRecipeResult>>).
{nbsp} +
{nbsp}
...
Format: `edit INDEX [name/RECIPE_NAME] [ingredient/INGREDIENT] [instruction/INSTRUCTION] [cooking_time/COOKING_TIME] [preparation_time/PREPARATION_TIME] [calories/CALORIES] [servings/SERVINGS] [url/LINK] [tag/TAG]`
...
To use this format, you do a `search` first (See <<Search>>) then select a recipe. +
...
Searchs for your recipe on the Internet.
...
* The index refers to the index number shown in the most recent listing.
* The index *must be a positive integer*, e.g. "1, 2, 3, ...".
* The text in the recipe will be copied to the clipboard for the User to paste into the Facebook post.
* The Facebook authentication page is shown on the browser for the User to log in.
* Upon logging in: if the shared recipe has a URL, the in-app browser will load a feed dialog for the User to share the URL as shown below:
image::Facebook_Feed_Dialog.jpg[width="400"]
...
* *`find Chicken`* +
*`share 1`* +
Shares the 1st recipe in the results of the *`find`* command.
...
* *_FILENAME_* must not start with / or \ or have consecutie slashes.
* *_FILENAME_* will be concatenated with .xml to maintain formatting.
* If *_FILENAME_* already exists in the user's Dropbox, one of two things will happen:
...
* *`pull chicken rice`* +
Downloads all recipes that match the query `Chicken rice` from the server.
...
.2+|    *Add*               |   `add name/RECIPE_NAME [ingredient/INGREDIENT] [instruction/INSTRUCTION] [cooking_time/COOKING_TIME] [preparation_time/PREPARATION_TIME] [calories/CALORIES] [servings/SERVINGS] [url/LINK] [tag/TAG]...`
...
.2+|    *Delete*            |  `delete INDEX`
                            |   E.g. *`delete 3`*
.2+|    *Edit*              |  `edit INDEX [name/RECIPE_NAME] [ingredient/INGREDIENT] [instruction/INSTRUCTION] [cooking_time/COOKING_TIME] [preparation_time/PREPARATION_TIME] [calories/CALORIES] [servings/SERVINGS] [url/LINK] [tag/TAG]`
...
| kbd:kbd:[&uarr;]          |   Previous command
| kbd:kbd:[&darr;]          |   Next command
| kbd:[Shift] + kbd:[Enter] |   New line in command
| kbd:[Shift] + kbd:[Space] |   Show command suggestions
