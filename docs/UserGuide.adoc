...
. If it is your first time to use ET, you are supposed to authorize ET to access your Google Calendar. Type command `authenET`, a web page will be opened in your default browser,
...
[NOTE]
====
* Employees Tracker is locked when Employees Tracker is opened.
...
* `unlock` +
A dialog will pop up like below in Figure 8. Fill in the input box with matched unlock password to unlock ET.
...
[NOTE]
====
This command will automatically copy the file you chosed to the application's resource folder if it does not exist. Once
...
Get the permission to allow Employees Tracker to access your google calendar. +
Format: `authenET`

=== Listing all employees : `list`

Shows a list of all employees in Employees Tracker. +
Format: `list`
...
****
* Make sure that the employees.csv is not open in your computer before excute `export`.
****
...
A pop-up dialog box will appear after user executes the command as shown in _Figure 8_ below. The dialog box will prompt the user to enter the user's credential (as a reviewer) and the review.
* `find n/Alex` +
`rate 2` +
...
[NOTE]
====
* You need to enter both old adn new password in a pop-up dialog, and old password must match the application's unlock password.
...
* `setPassword` +
A dialog will pop up like below in Figure 12. Fill in the input boxes with matched old password and new password to set it as new password.
