...
public class AddCommandParserTest {
    ...
    public void parse_allFieldsPresent_success() {
        Recipe expectedRecipe = new RecipeBuilder().withName(VALID_NAME_BOB).withPreparationTime(VALID_PREPARATION_TIME_BOB)
                .withIngredient(VALID_INGREDIENT_BOB).withInstruction(VALID_INSTRUCTION_BOB).withTags(VALID_TAG_FRIEND).build();

        // whitespace only preamble
        assertParseSuccess(parser, PREAMBLE_WHITESPACE + NAME_DESC_BOB + PREPARATION_TIME_DESC_BOB + INGREDIENT_DESC_BOB
                + INSTRUCTION_DESC_BOB + TAG_DESC_FRIEND, new AddCommand(expectedRecipe));

        // multiple names - last name accepted
        assertParseSuccess(parser, NAME_DESC_AMY + NAME_DESC_BOB + PREPARATION_TIME_DESC_BOB + INGREDIENT_DESC_BOB
                + INSTRUCTION_DESC_BOB + TAG_DESC_FRIEND, new AddCommand(expectedRecipe));

        // multiple preparationTimes - last preparationTime accepted
        assertParseSuccess(parser, NAME_DESC_BOB + PREPARATION_TIME_DESC_AMY + PREPARATION_TIME_DESC_BOB + INGREDIENT_DESC_BOB
                + INSTRUCTION_DESC_BOB + TAG_DESC_FRIEND, new AddCommand(expectedRecipe));

        // multiple ingredients - last ingredient accepted
        assertParseSuccess(parser, NAME_DESC_BOB + PREPARATION_TIME_DESC_BOB + INGREDIENT_DESC_AMY + INGREDIENT_DESC_BOB
                + INSTRUCTION_DESC_BOB + TAG_DESC_FRIEND, new AddCommand(expectedRecipe));

        // multiple instructions - last recipe accepted
        assertParseSuccess(parser, NAME_DESC_BOB + PREPARATION_TIME_DESC_BOB + INGREDIENT_DESC_BOB + INSTRUCTION_DESC_AMY
                + INSTRUCTION_DESC_BOB + TAG_DESC_FRIEND, new AddCommand(expectedRecipe));

        // multiple tags - all accepted
        Recipe expectedRecipeMultipleTags = new RecipeBuilder().withName(VALID_NAME_BOB).withPreparationTime(VALID_PREPARATION_TIME_BOB)
                .withIngredient(VALID_INGREDIENT_BOB).withInstruction(VALID_INSTRUCTION_BOB)
                .withTags(VALID_TAG_FRIEND, VALID_TAG_HUSBAND).build();
        assertParseSuccess(parser, NAME_DESC_BOB + PREPARATION_TIME_DESC_BOB + INGREDIENT_DESC_BOB + INSTRUCTION_DESC_BOB
    ...
    public void parse_optionalFieldsMissing_success() {
        // zero tags
        Recipe expectedRecipe = new RecipeBuilder().withName(VALID_NAME_AMY).withPreparationTime(VALID_PREPARATION_TIME_AMY)
                .withIngredient(VALID_INGREDIENT_AMY).withInstruction(VALID_INSTRUCTION_AMY).withTags().build();
        assertParseSuccess(parser, NAME_DESC_AMY + PREPARATION_TIME_DESC_AMY + INGREDIENT_DESC_AMY + INSTRUCTION_DESC_AMY,
    ...
    public void parse_compulsoryFieldMissing_failure() {
        ...
        // missing name prefix
        assertParseFailure(parser, VALID_NAME_BOB + PREPARATION_TIME_DESC_BOB + INGREDIENT_DESC_BOB + INSTRUCTION_DESC_BOB,
                expectedMessage);

        // missing preparationTime prefix
        assertParseFailure(parser, NAME_DESC_BOB + VALID_PREPARATION_TIME_BOB + INGREDIENT_DESC_BOB + INSTRUCTION_DESC_BOB,
                expectedMessage);

        // missing ingredient prefix
        assertParseFailure(parser, NAME_DESC_BOB + PREPARATION_TIME_DESC_BOB + VALID_INGREDIENT_BOB + INSTRUCTION_DESC_BOB,
                expectedMessage);

        // missing recipe prefix
        assertParseFailure(parser, NAME_DESC_BOB + PREPARATION_TIME_DESC_BOB + INGREDIENT_DESC_BOB + VALID_INSTRUCTION_BOB,
                expectedMessage);

        // all prefixes missing
        assertParseFailure(parser, VALID_NAME_BOB + VALID_PREPARATION_TIME_BOB + VALID_INGREDIENT_BOB + VALID_INSTRUCTION_BOB,
                expectedMessage);
    }
    ...
    public void parse_invalidValue_failure() {
        // invalid name
        assertParseFailure(parser, INVALID_NAME_DESC + PREPARATION_TIME_DESC_BOB + INGREDIENT_DESC_BOB + INSTRUCTION_DESC_BOB
                + TAG_DESC_HUSBAND + TAG_DESC_FRIEND, Name.MESSAGE_NAME_CONSTRAINTS);

        // invalid preparationTime
        assertParseFailure(parser, NAME_DESC_BOB + INVALID_PREPARATION_TIME_DESC + INGREDIENT_DESC_BOB + INSTRUCTION_DESC_BOB
                + TAG_DESC_HUSBAND + TAG_DESC_FRIEND, PreparationTime.MESSAGE_PREPARATION_TIME_CONSTRAINTS);

        // invalid ingredient
        assertParseFailure(parser, NAME_DESC_BOB + PREPARATION_TIME_DESC_BOB + INVALID_INGREDIENT_DESC + INSTRUCTION_DESC_BOB
                + TAG_DESC_HUSBAND + TAG_DESC_FRIEND, Ingredient.MESSAGE_INGREDIENT_CONSTRAINTS);

        // invalid recipe
        assertParseFailure(parser, NAME_DESC_BOB + PREPARATION_TIME_DESC_BOB + INGREDIENT_DESC_BOB + INVALID_INSTRUCTION_DESC
                + TAG_DESC_HUSBAND + TAG_DESC_FRIEND, Instruction.MESSAGE_INSTRUCTION_CONSTRAINTS);

        // invalid tag
        assertParseFailure(parser, NAME_DESC_BOB + PREPARATION_TIME_DESC_BOB + INGREDIENT_DESC_BOB + INSTRUCTION_DESC_BOB
                + INVALID_TAG_DESC + VALID_TAG_FRIEND, Tag.MESSAGE_TAG_CONSTRAINTS);

        // two invalid values, only first invalid value reported
        assertParseFailure(parser, INVALID_NAME_DESC + PREPARATION_TIME_DESC_BOB + INGREDIENT_DESC_BOB + INVALID_INSTRUCTION_DESC,
                Name.MESSAGE_NAME_CONSTRAINTS);

        // non-empty preamble
        assertParseFailure(parser, PREAMBLE_NON_EMPTY + NAME_DESC_BOB + PREPARATION_TIME_DESC_BOB + INGREDIENT_DESC_BOB
