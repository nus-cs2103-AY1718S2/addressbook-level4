...
* If you want to view the subjects of the student, please refer to <<Selecting a student : `select` or `s`>>
...
* If you have typed in duplicate subjects, the system will return an error message as shown in Figure... +
You can remove the duplicate subject and press `Enter` to add the subject(s).
...
* `addsub 1 sub/Music A1 French A1` +
Adds subjects `Music A1` and `French A1` to the student at `INDEX` 1.
// end::addsubject[]
...
Adds a CCA and the student's position in the CCA into the student records in _EduBuddy_. +
Format: `addcca INDEX cca/CCA pos/POSITION` +
You will be able to see the success message, +
`CCA added: CCA: POSITION` +
`Person: NAME`

[NOTE]
Adds a CCA and the student's position in the CCA to the student at the specified `INDEX`. +
The index refers to the index number shown in the most recent listing.
...
* `list` +
`addcca 2 cca/Basketball pos/Member` +
Adds the cca basketball with the position member to the 2nd student in _EduBuddy_.
* `find Betsy` +
`addcca 1 cca/Choir pos/Member` +
Adds the cca Choir with the position member to the 1st student in the results of the `find` command.

*_return to <<toc,Table of Contents>>_*

=== Deleting a CCA from a student : `deletecca` [since v1.5]
Deletes a CCA and the student's position in the CCA from the student records in _EduBuddy_. +
Format: `deletecca INDEX cca/CCA pos/POSITION` +
You will be able to see the success message, +
`CCA deleted: CCA: POSITION` +
`Person: NAME`

[NOTE]
Deletes a CCA and the student's position in the CCA from the student at the specified `INDEX`. +
The index refers to the index number shown in the most recent listing.
...
* `list` +
`deletecca 2 cca/Basketball pos/Member` +
Deletes the cca basketball with the position member from the 2nd student in _EduBuddy_.
* `find Betsy` +
`addcca 1 cca/Choir pos/Member` +
Deletes the cca Choir with the position member from the 1st student in the results of the `find` command.
...
For more information about CCA score, click https://www.moe.gov.sg/docs/default-source/document/education/programmes/co-curricular-activities/leaps-2.pdf[here] to find out more.
...
For more information about LEAPS, please click https://www.moe.gov.sg/docs/default-source/document/education/programmes/co-curricular-activities/leaps-2.pdf[here]
