...
The sending of email will be implemented using the Gmail API.

Firstly, `GoogleAuthorizationCodeFlow` is used to build and request for authorization from user.
...
This authorization is handled in the `GmailAuthentication` class.

Next, a `GmailClient` is created using the `Credential` that was created by `GmailAuthentication`.
The `GmailClient` class provides all the services that uses the Gmail API.
...
the authorised user's email address, which we can retrieve using `GmailClient`.

The `GmailMessage` class will take in the values inputted by the user
