...
The main component of 'findtag' is the 'FindWithTagCommand' which resides inside the logic component. It inherit from 'Command', as such, similar to 'find', 'findtag' is not an undoable command. The following diagram shows the inheritance diagram for 'FindWithTagCommand':
...
image::FindTagResultDiagram.png[width="800"]
...
image::InvalidCommandKeywordDiagram.png[width="800"]

Note that the list of persons displayed will not change for in this case.
...
** Cons: May generate classes/ methods that are implemented with very similar style (i.e. a general pattern can be abstract out). Thus, additional work may be needed to handle the abstraction.
...
command.
This feature allows a user to have the application keep track of all the links to the social media platform profiles of
