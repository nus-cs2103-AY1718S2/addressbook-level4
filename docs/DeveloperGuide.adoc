...
It is an important class in this application as it represent each entry in the desk board.
In CLIndar, an entry in the desk board can be either a `Task` or an `Event`.

==== Current Implementation
_Figure 11_ below shows the relationship between `Activity` and its relevant components.
...
_Figure 12_ below shows how `Task` is represented in the application.
...
_Figure 13_ below shows how `Event` is implemented in the application.
...
*_Figure 14_* below shows how `Task` is represented in the application.
...
* Name: name of the task. This attribute is compulsory.
* dueDateTime: due date and time of the task. This attribute is compulsory.
* Remark: a short description or reminder pertaining to the task.
* Tags: Groupings or representation for the task.

Operations that manipulate `Task` can been done using `TaskCommand`, `CompleteCommand`,
...
* `INFO` shows noteworthy actions by the App.
