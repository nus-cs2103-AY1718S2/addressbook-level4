= image:../../src/main/resources/images/displayscreenimage.png[width="80"] contactHeRo - Developer Guide
:toc:
:toc-title:
...
image::Architecture.png[width="800", align="center"]
...
// tag::undoredo[]
=== Undo/Redo feature
Given below is the implementation detail of the Undo/Redo feature and some alternative design considetations.
...
Given below is the implementation detail of the Details Panel and some alternative design considetations.
...
The `select` command shows the contact details of the person at the specified index under the `Contact Details` tab.
The `ContactDetailsDisplay` is a GridPane embedded in the `Contact Details` tab and the UML diagram below represents the UI structure for the `ContactDetailsPanel` (Refer to Figure 30):

.UML Diagram for DetailsPanel UI
...
.Wireframe for Contact Details Panel.
...
Given below is the implementation detail of the Tab Switching Mechanism and some alternative design considetations.

==== Current Implementation
Tab Switching Mechanism is an event driven mechanism.

.Component interaction for Tab Switching Mechanism.
image::SwitchTabRequestEventHighSD.png[width="800", align="center"]
...
.Sequence diagram for the event posting for the Tab Switching Mechanism.
image::SwitchTabRequestEventSD.png[width="800", align="center"]
...
.Sequence diagram for the event handling for the Tab Switching Mechanism.
image::SwitchTabRequestEventSD2.png[width="350", align="center"]
...
Given below is the implementation detail of the Job Openings feature and some alternative design considetations.
...
.UML class diagram for Job Opening
image::JobUML.png[width="600", align="center"]
...
.Model management of Job objects
image::JobModelUML.png[width="600", align="center"]
...
.High Level Sequence Diagram for adding a job opening
image::SDforAddJobEventHandling.png[width="800", align="center"]
...
.UML class diagram for Job Opening
image::Auto-CompleteActivityDiagram.png[width="800", align="center"]

The `previous input` and `tab Counter` are recorded to account for cases where more than one command share the same
prefix. For e.g. the `add`, `addjob` and `addapp` commands share the common `add`. In such a case, the user can just
type `a` or `ad` or `add` and press `Tab` multiple times to cycle between these commands which will keep incrementing
the Tab counter, `tabNumber` in a cyclic manner.
...
The `EmailCommand` class will open an email interface to send email to the chosen contact. `EmailCommand` extends the `Command` class. It takes in an index and also an optional field `EMAILSUBJECT` through the `EmailCommandParser`,
similar to the implementation of `EditCommand` and `EditCommandParser`.
...
Most of the commands will be locked so as to prevent modification of the data in contactHeRo before a user logs in. The commands that are available for use are `LoginCommand`, `HelpCommand`, `ClearHistoryCommand` and `ExitCommand`.

The user will need to enter the `login` command to log in before he can get to the main window. The image below shows
the sequence diagram for the `login` command in the logic component.

.Logic Component Sequence Diagram for logging in
image::LoginSDLogicComponent.png[width="800", align="center"]

As seen in the image, when the user logs in, an instance of `LogicCommand` is created which then makes a function call to `Model` to check the validity of the inputs. Afterwards, the `LogicCommand` instance will post a `LoginEvent` to \
`EventsCenter` and the event will be handled by the `UI` component to switch the display.
...
.LoginScreen.png
image::LoginPrompt.png[width="800", align="center"]
...
**Alternative 1 (current choice):** Stores user account data in the same file with the main data. +
**Pros**: +
**Cons**: Data in contactHeRo becomes lost if user forgets the customized username or password. +

**Alternative 2:** Stores user account data in a separate file.+
**Pros**:  User can still retrieve their data if they forget their customize username or password by simply removing the file storing account data and restart with a +
**Cons**: Data becomes insecure as now everyone can access the data through the default username and password. +
...
.. Other incorrect delete commands to try: `delete`, `delete x` (where x is larger than the list size), `delete -1` (negative index) +
...
.. Test case: `addjob p/Job1 t/Team1 l/Singapore n/1 s/Java` +
...
.. Test case: `addjob p/Job2 t/Team2 l/Malaysia` +
   Expected: No job opening is added. Error details shown in the status message. Status bar remains the same.
.. Other incorrect commands to try: `addjob p/Job3 t/Team3 l/Malaysia n/abc s/C++` (invalid number of positions), `addjob p/Job2 t/Team2 l/Malaysia n/1 s/` (empty skills) +
   Expected: Similar to previous.

=== Deleting a job opening

. Deleting a job opening while all job openings are listed

.. Prerequisites:
** List all job openings using the `listjob` command.
** There must be multiple job openings in the list.
.. Test case: `deletejob 2` +
...
.. Other incorrect deletejob commands to try: `deletejob`, `deletejob x` (where x is larger than the list size), `deletejob -1` (negative index) +
    Expected: Similar to previous.

. Deleting a job opening by finding it

.. Prerequisites:
** Job opening to find must exist in the job opening list.
.. Test case: `findjob p/Software` followed by `deletejob 1` +
...
.. Test case: `findjob p/Software` followed by `deletejob 4` +
...
.. Test case: `editjob 1 p/Hardware Engineer` +
...
.. Test case: `editjob 2 p/Hardware Engineer t/Cloud Services l/Singapore n/2 s/Java s/Algorithms` +
...
.. Other incorrect editjob commands to try: `editjob`, `editjob 1`, `editjob 1 p/` (empty field) +
...
.. Test case: `findjob l/New York p/Engineer` +
...
.. Other incorrect findjob commands to try: `findjob`, `findjob Engineer` (without prefix), `findjob 1 p/` (empty field) +
Expected: Job openings are not searched. Error details shown in the status message. Status bar remains the same.

=== Matching a job opening
...
.. Other incorrect matchjob commands to try: `matchjob`, `delete x` (where x is larger than the list size), `matchjob Engineer` (matching by position) +
Expected: Job openings are not searched. Error details shown in the status message. Status bar remains the same.
...
.. Test case: `linkedIn 0` +
   Expected: No person is searched on LinkedIn. Error details shown in the status message. Status bar remains the same.
.. Other incorrect linkedIn commands to try: `linkedIn`, `linkedIn x` (where x is larger than the list size), `linkedIn -1` (negative index), `linkedIn Alex` (by name) +
   Expected: Similar to previous.

=== Auto-completing the command

. Auto-completing the command

.. Prerequisites: None.
.. Test case: Type `a` and press `Tab`. +
   Expected: The command is auto-completed to `add` with its syntax.
.. Test case: Type `l` and press `Tab` repeatedly. +
   Expected: The auto-completed command cycles between `linkedIn`, `list`, `listjob` and `login`.
.. Test case: Type `ab` and press `Tab`. +
...
.. Prerequisites: Having a profile picture path, a non image file path and a non existing profile picture path. Let's call them `profile_picture_path`, `invalid_profile_picture_path` and `non_exist_profile_picture_path`. +
.. Test case: `add n/Tester p/98765432 e/tester@example.com a/NUS cp/Developer cc/Google pp/profile_picture_path`. +
   Expected: contact Tester is added with the profile picture corresponding to `profile_picture_path`.
.. Test case: `add n/Tester2 p/98765432 e/tester@example.com a/NUS cp/Developer cc/Google`. +
   Expected: contact Tester2 is added with default profile picture.
.. Test case: `add n/Tester3 p/98765432 e/tester@example.com a/NUS pp/invalid_profile_picture_path`. +
   Expected: No person is added. Error details shown in the status message. Status bar remains the same. +
.. Test case: `add n/Tester3 p/98765432 e/tester@example.com a/NUS pp/non_exist_profile_picture_path`. +
   Expected: No person is added. Error details shown in the status message. Status bar remains the same.
. Edit a profile picture.
.. Prerequisites: same as above and at list one person in list.
.. Test case: `edit 1 pp/profile_picture_path`. +
...
.. Test case: `edit 1 pp/invalid_profile_picture_path`.
   Expected: Person 1 is not edited. Error details shown in the status message. Status bar remains the same. +
.. Test case: `edit 1 pp/non_exist_profile_picture_path`.
   Expected: Person 1 is not edited. Error details shown in the status message. Status bar remains the same. +

=== Managing appointments
. Adding/deleting an appointment.

.. Prerequisites: appointment with `t/Discussion sdt/2018-05-26 12:00 edt/2018-05-26 12:30` has not been added. Test in the order below. +
.. Test case: `addapp t/Discussion sdt/2018-05-26 12:00 edt/2018-05-26 12:30` +
...
.. Test case: `delapp t/Discussion sdt/2018-05-26 12:00 edt/2018-05-26 12:30`  +
...
.. Test case: `addapp t/Discussion sdt/2018-05-26 12:00 edt/2018-05-25 12:30`
...
.. Test case: `date` or `week` or `month` or `year`. +
   Expected: corresponding view is shown.
.. Test case: `datetime 2018/03/26 12:00` or `date 2018/03/26` or `week 2018 10` or `month 2018/03` or `year 2018`.
   Expected: corresponding view at specific time is shown.
.. Test case: `datetime 2018/02/29 12:00`. +
...
. Logging in to Google using the command
.. Type `googlelogin` into the command box.
.. The display panel should switch to the `Google` tab and the Google Account Login page should be displayed.
.. Login using your Google account, click `Allow` to allow contactHeRo some access to your Gmail account.
.. After you have logged in, you should be redirected to the Google homepage.

=== Sending an email

. Sending Email to contact

.. Prerequisites: Logged in to Google using the command `googlelogin`.
.. Test case: `email 1` +
   Expected: First contact's email address is displayed in the `To:` textbox.
   First Contact's name is used in the template "Dear [NAME]" in the `Body:` textbox.
.. Test case: `email 1 sub/Interview on 13 May` +
   Expected: First contact's email address is displayed in the `To:` textbox.
   First Contact's name is used in the template "Dear [NAME]" in the `Body:` textbox.
   The subject title "Interview on 13 May" is displayed in the `Subject:` textbox.
.. Test case: clicking `Send` +
   Expected: Email should be sent to the correct email address, a pop-up box should
   be displayed to show that you have sent the email successfully.
