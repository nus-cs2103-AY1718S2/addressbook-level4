...
Events management is facilitated by `UserEvents`, which resides inside `Model`.
...
In the Figure 14, `ModelManager` creates an instance of `UserEvents` interface, which provides the `UserEvents` API for `ModelManager` to implement.
...
In Figure 15, when a user enters the command to add an event, `AddEventCommand` will call `addEvent()` in `ModelManager`.
