...
The user executes a new `UndoableCommand`, `delete 5`, to delete the 5th task in the address book. The current state of the address book is saved before the `delete 5` command executes. The `delete 5` command will then be pushed onto the `undoStack` (the current state is saved together with the command).

image::UndoRedoStartingStackDiagram.png[width="800"]

As the user continues to use the program, more commands are added into the `undoStack`. For example, the user may execute `add n/David ...` to add a new task.
...
. Fix `invalidAndValidPersonAddressBook.xml`, `typicalPersonsOrganizer.xml`, `validAddressBook.xml` etc., such that the XML tests will not fail due to a missing `<remark>` element.
...
* prefers a digital organiser over a physical organiser
* would rather the application do the planning for them

*Value proposition*: helps busy university students keep track of their tasks and priorities
...
|`* * *` |user |add a priority level to a task |allow the application to order tasks by priority level

|`* * *` |user |add a deadline to a task |allow the application automate management of my task priorities
...
|`* *` |user |delete a tag from a task |remove a tag from a task that no longer belongs to a group
...
|`* * *` |user |edit a task's name |update the task's name in case of changes needed

|`* * *` |user |edit a task's priority level |change a task's priority level when I need to reorganize priorities
...
|`* * *` |user |mark a task as complete |indicate to the application to push a task to the completed list

|`* *` |user |mark a subtask as complete |keep track of subtasks already done

|`* * *` |user |view all uncompleted tasks |see all tasks that need to be done, along with the deadlines and priorities

|`* * *` |user |view all details of a task |view all details of a specified tasks, after seeing a list of all tasks

|`* * *` |user |view all completed tasks |see all tasks that have been completed, for past reference

|`* *` |user with many tasks in the to-do list |view all tasks with a specific tag |locate all tasks with a specific tag, without the need of a manual search

|`* *` |user with many tasks in the to-do list |view all tasks with a certain deadline |locate all tasks with a certain deadline, without the need of a manual search

|`* *`|user |view a calender showing task's deadlines|see a chronic overview of all tasks and their deadlines

|`* *` |user with many tasks in the to-do list |find a task by name|locate a task without the need to search through a list

|`* *` |user with many tasks in the to-do list |find a task by description|locate a task when I have forgotten it's name

|`* *` |user |have all tasks' priority levels be automatically updated as the deadline approaches |transfer the responsibility of updating priorities to the application

|`* *` |user |have a task's priority level automatically increased if still uncompleted after the deadline has passed |be reminded of urgent uncompleted tasks

|`* *` |user |be reminded of tasks of highest priority |remember to complete tasks of highest priority

|`* *` |user |undo an operation |revert an operation without having to think of it's opposite operation

|`* *` |user |redo an undo operation |

|`* *` |user |automatically set tasks with no priority level to the lowest level |have priority levels for all tasks even if I had forgotten to set them

|`*` |user |change view of the calendar |choose to view the calendar by year, month, week, or day
