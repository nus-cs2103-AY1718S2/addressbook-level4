...
It is customised for students in the National University of Singapore (NUS) which allows them to find vacant rooms within NUS and also to track their timetable.
...
=== Birthdays feature
==== Current Implementation
Birthdays Command uses the existing Events system and stacks a additional StackPane over the existing BrowserPane.

Calling `birthdays` command will create a event, which retrieves the current UniquePersonList from Model as an observableList to the UI
The BirthdayList UI class then converts the list into an editable list.
It further parses the day and month of the given Birthday and sorts the list.
Afterwards, it brings the birthdayPlaceholder to the front, replacing the browserPlaceholder.
...
** Cons: Not apparent in usage by User. Functionality remains the same but code becomes more coupled.
* **Alternative 2: Use Natural Language Processing for parsing Birthday
** Pros: Users can enter their birthday in any format
...
** Cons: Currently, StardyTogether does not have settings to switch on/off the feature if implemented. May clash with encryption.
...
|Wayne Neo| Designing UI for Timetable. UI will display the timetable of each person and also the union of timetables to find common timeslots| Adding of Birthday class into the `Person` class. With an additional command, it allows users to generate a list of birthdays to celebrate with their friends.
|=======================================================================
...
|`* * *` |busy student |have my own shortforms of commands |type more quickly and in my own style
