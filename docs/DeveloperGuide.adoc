...
When a person is selected, either by the `select` command or by mouse click via the `UI`, a `PersonPanelSelectionChangedEvent`. The flow of operation thereafter is shown the sequence diagram below.
...
. Adding a person specified by command line input with required person prefixes.
...
.. Test case: `add n/John Doe p/98765432 e/johnd@example.com a/311, Clementi Ave 2, #02-25 l/http://modsn.us/MYwiD d/Likes tennis t/friends t/owesMoney` +
   Expected: New person John Doe is added to Fastis. Details of person added is shown in the status message.
.. Test case: `add p/98765432 e/johnd@example.com a/311, Clementi Ave 2, #02-25 l/http://modsn.us/MYwiD d/Likes tennis t/friends t/owesMoney` +
   Expected: No person is added. Essential Name field is missing. Error details is displayed in status message.
.. Other incorrect add commands to try: `add`, `add x` (where any input) +
   Expected: Similar to previous.

=== Adding a group

. Adding a group with information specified by command line input.

.. Prerequisites: Group with the information specified by user must not be in Fastis.
.. Test case: `addGroup CS1010` +
   Expected: New group with information `CS1010` will be added to Fastis. Successful adding of group `CS1010` will be reflected in status message.
.. Test case: `addGroup CS1-1-` +
   Expected: No group is added. Group information must be alphanumeric. Error details is displayed in s status message.
.. Other incorrect commands: `addGroup CS!` +
   Expected: Similar to previous.
.. Group `CS1010` already existed. +
   Test case:`addGroup CS1010` +
   Expected: Error details is displayed in status bar that group already existed.

=== Adding a member to group

. Adding a member identified by `Index` to group identified by `Information`.

.. Prerequisites: Group and Person both exist in Fastis. Person must exist on current Person List.
.. Test case: `addGroupMember 1 g/CS1010` +
   Expected: Member that is first on Person List will be added to group with information `CS10101. Successful addding of person will be displayed on status bar.
.. Test case: `addGroupMember 1 CS1010` +
   Expected: No member is added as invalid command input since group field `g/` is missing. Error details is disp;ayed in status bar.
.. Other incorrect commands to try: `addGroupMember`, `addGroupMember 1 t/` +
   Expected: Same as previous.

=== Deleting a member from group

. Deleting a person from a group specified by user. Often use after `listGroupMembers` command to see which members are in the group.

.. Prerequisites: Group and Person both exist in Fastis. Person must exist on current Person List.
.. Test case: `deleteGroupMember 1 g/CS1010` +
   Expected: Member is successfully deleted from the group `CS1010`. Successful deletion is displayed on the status bar.
.. Test case: `deleteGroupMember 1 CS1010` +
   Expected: No member is deleted due to missing field in command. Error details is displayed in the status bar.
.. Other incorrect commands to try: `deleteGroupMember`, `deleteGroupMember 1 t/` +
   Expected: Same as previous.

=== Listing group members from a group

. Listing all persons from a group specified by user.

.. Prerequisites: Group must exist in Fastis.
.. Test case: `listGroupMembers CS1010` +
   Expected: All members in group with information `CS1010` is listed on Person List. Successful listing is displayed on the status bar.
.. Test case: `listGroupMembers !` +
   Expected: No person is listed. Error details is displayed in the status bar.

=== Deleting a group

. Deleting a group with information specified by user from Fastis.

.. Prerequisites: Group must exist in Fastis.
.. Test case: `deleteGroup CS1010` +
   Expected: Group with information `CS1010` is deleted. Successful deletion is displayed in status bar.
.. Test case: `deleteGroup @` +
   Expected: No group is deleted. Error details is displayed in the status bar.

=== Listing members with same tag

. Listing members with same tag

.. Prerequisites: Tags must exist in Fastis.
