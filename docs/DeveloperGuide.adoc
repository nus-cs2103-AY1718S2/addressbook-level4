...
By: `Team T16-B2`      Since: `Mar 2018`      Licence: `MIT`

Special Credits to: AddressBook-Level4 project created by SE-EDU initiative at https://github.com/se-edu/
...
.  The command execution can affect the `Model` (e.g. adding a person) and/or raise events. Especially, the distance command also depend on the Google Map API.
...
The `login` feature thus has limited interactions with the other components of the application after successful verification.
This, however, may change in v2.0 when encryption of the Pigeons address book is built in and integrated with the profile created by the `login` component.
...
We use an `Autocompleter` class which stores the list of `Command` and all the different fields (`Name`, `Address`, etc) created in the `LogicManager`.
The `Autocompleter` can complete:

* a command word, if it's the first of a query
* an option, starting by "n/" for example
* a name, if the current query doesn't match the first two cases: if we only have "John Doe" in the `AddressBook` we can complete the query by either "John" or "Doe" (this is useful to complete the `FindCommand`).

After having chosen between one this three cases, we generate a set of all possible suffixes that can be append to the current query. And from this set, we extract the longest common prefix which is the returned `String` of `Autocompleter.autocomplete()`. +
For example : if we want to complete the query `find J` and we have two persons in the address book called `John` and `Jonathan`, the set of possible suffixes will be {`ohn`, `onathan`} and the longest common prefix is `o`.
So the completed query will be `find Jo`.

This class is used in `CommandBox` which sends the content of the command line and then append the result of the autocomplete operation.
// end::autocompletion[]

//tag::mailer[]
=== Mailer feature

We use an abstract `Mailer` class which provide a method that sends an email to a list of `Person`
and another which sends an itinerary to the driver. +
This class uses the JavaMail API to send emails from the address : pigeonscs2103@gmail.com. +
We also created a new `EmailCommand` which directly uses the `Mailer` class in its redefinition of `super.execute()`.
The information related to the itinerary are collected by the `EmailCommand` which creates an instance of `RouteOptimization`.
//end::mailer[]
...
Delivery system for drivers with a database of deliveries and locations to map the optimal route in a handsfree offline system.

*Target user profile*:

* has a need to manage deliveries
* prefer desktop apps over other types
* requires audio cues while driving and using the program
* prefers typing over mouse input
* is reasonably comfortable using CLI apps
* visually-oriented so prefers to see maps instead of addresses
* may take on different user profiles for different users with different contact lists
* may require to add contacts on the go occasionally quickly
* may like to be able to communicate with addresses

*Value proposition*: manage deliveries faster than a typical GUI driven app while being accessible easily on the go and
maps out ideal routes for drivers
...
Minor: UI for overall application

Major: Login for each delivery driver to own a separate list of addresses in his customised address book

*Meera*

Minor: adding delivery date field

Major:  Route optimization for the deliveries on each date

*Matthieu*

Minor : simplifying the input

Major: Email updates/correspond with the users that are awaiting a delivery

*Minh*

Minor: Adding google maps and handsfree sound feature

Major: Mapping out the routes for delivery and displaying on google maps
...
|`* * *` |user |add a new person | add entry into my address book

|`* * *` |user |delete a person |remove entries that I no longer need

|`* * *` |user |find a person by name |locate details of persons without having to go through the entire list

|`* * *` |user |*have a separate address book for different user profiles* |minimize chance of someone else seeing my address book by accident

|`* * *` |user with many persons in the address book |sort persons by name, address |locate a person easily

|`* * *` |user with many deliveries in the address book |sort deliveries by date, priority |make my planning

|`* * *` |driver |quickly indicate that I have completed a delivery |always know which are the remaining parcels that I have to deliver

|`* * *` |driver |*have a map view of the route to my next customer* | easily figure out the way I should take

|`* * *` |driver |*be able to communicate with the customer* |resolve any delivery issues I face

|`* * *` |customer |receive updates about my parcel |know when it's gonna come

|`* * *` |driver |*be able to find the optimal route to the next destination easily* |focus on just driving to the next location

|`* *` |driver| schedule my whole day by ordering the deliveries I want to do |avoid losing time during the day

|`* *` |driver |have users automatically downloaded from a central database |avoid having to add users one by one myself

|`* *` |driver |get traffic updates |follow my schedule

|`*` |customer |real-time geo-locate my delivery | to have a better idea of when my delivery will arrive
|=======================================================================

_{User stories are added/updated subsequently through issues in GitHub as the project develops through iterative feedback}_
...
// tag::usecases[]
[discrete]
=== Use case: Deliver a package to a specific customer

*MSS*

1. User enters the package reference
2. Address book outputs the customer's information (availabilities, address)
3. User clicks on the customer’s address
4. Address book open the itinerary to the customer’s address on Google Maps
5. User can drive to the customer’s location
+
Use case ends.

*Extensions*

* 2a. The given package reference is invalid.
+
[none]
...
[discrete]
=== Use case: Mapping the route for the day

*MSS*

1. User requests to list the database ordered by date of delivery
2. Address book outputs the list of deliveries
3. User selects all the addresses and runs the “map” command on the selected addresses
4. Address book displays a mapped out route for the day with an ideal time to begin the route
5. User can start the delivery
+
Use case ends.
// end::usecases[]
...
.  Need to have access to the app while driving, needs to work offline and have internet access for any updates during the day
.  Driver needs to be trained in the tool so it should be user-friendly
...
[appendix]
== Product Survey
Below is a sample of how a product survey may be documented:
...
.. Double-click the jar file +
   Expected: Shows the login screen GUI, and after successful login, it shows GUI with a set of sample contacts. The window size may not be optimum.
.. Key in the username and password. Create a new account if you don't have one. +
   Expected after logging in: The system speaks out "Welcome user" (for Window and Mac only) and the Pigeon application is now ready to be used.
...
.. Delete the data folder or corrupted file and repeat commands. A new data folder/file should be auto-generated and auto-saved again.

// tag::testScript[]
=== Virtual assistant
. Reads out a person name and show route from the Headquarter to the person address:
