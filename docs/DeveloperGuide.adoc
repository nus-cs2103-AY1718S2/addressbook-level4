...
`Review` uses JavaFX's TextInputDialog to get the review input from the user instead of from command box.
...
. `ModelManager` and `UniquePersonList` provide sorting algorithm, called by `AddressBook`.
. `UniquePersonList` will sort the units by giving field
