...
This *Developer Guide* will take you through the design and implementation of *ReciRecipé*. If you want to understand _how_, and hopefully _why_, *ReciRecipé* works, you are at the right place!
...
Follow these step carefully to set up ReciRecipé on your computer.
...
IntelliJ should look like Figure 1 below after step 3.
...
. <<Design-Main,*`Main`*>>: Contains the app.
. <<Design-Commons,*`Commons`*>>: Contains classes that are used by multiple components.
...
* `_EventsCenter_` : Written using https://github.com/google/guava/wiki/EventBusExplained[Google's Event Bus library] and used by components to communicate with other components using events (i.e. a form of _Event Driven_ design).
...
{nbsp} +
For finer details concerning `_XYZCommand_` and `_Command_`, refer to <<fig-LogicCommandClassDiagram>>.
{nbsp} +
...
The command is implemented in `_ChangeThemeCommand_`. When user submits the command to the *`UI`* component, it passes on the command to the *`Logic`* component, which posts a `_ChangeThemeRequestEvent_` to the `_EventsCenter_` (refer to <<fig-ChangeThemeSD>>).
...
The event is then propagated through the `_EventsCenter_` to the `*UI*`. The `*UI*` will call `handleChangeThemeRequestEvent()` to update the theme accordingly (refer to <<fig-ChangeThemeEventSD>>).
...
* **Alternative 1 (current choice):** Reload the default page.
** Pros: It is easy to implement.
** Cons: Any page loaded on `_BrowserPanel_` will be lost during the operation.
...
* **Alternative 3:** Reload the default page *and* reload any page currently loaded on `_BrowserPanel_`.
** Pros: It is possible to keep the status of the `_BrowserPanel_`, in other words, any page loaded may still retain.
** Cons: This approach is obviously harder to implement, also it will sends an additional request to the server of the website.
* **Alternative 4:** Save any page currently loaded on `_BrowserPanel_`, reload the default page then load the saved page.
** Pros: This is the ideal output.
** Cons: This is the ideal output, thus very hard to implement.
* **Alternative 5:** Stop using the default page.
** Pros: It is somewhat easy to implement.
** Cons: Obviously there is no default page to display so we have to find another approach to make `_BrowserPanel_` display something as a placeholder, *without* using a default page.
// end::change-theme[]
...
* **Alternative 1 (current choice):** Save the access token within the application for future use within that session.
...
** Pros: Simple implementation can be used as a reference by new developers for creating optional fields. Interaction with `_BrowserPanel_` requires more control over the status of a recipe's URL property.
** Cons: `_Recipe_` class may contain unnecessary variables.
* **Alternative 2:** Store URL property in the same fashion as tag labels.
** Pros: `_Recipe_` class has better separation of optional and compulsory properties.
...
** Pros: User has more control over what is posted on their Facebook wall. Implementation is easier to maintain.
** Cons: User has to take more steps to successfully make a post (paste recipe onto post, click "post").
* **Alternative 2:** Use RestFB API.
** Pros: Recipe is posted immediately on the User's Facebook wall using the *`share`* command. User need not manually paste the recipe from the clipboard onto the post.
** Cons: User *must* manually obtain their Facebook access key and input it into the application - this may raise security concerns (Facebook does not allow automatic posting of text even if permission is given by the User upon log in).
...
** Pros: No need for a mouse. Ensures that the application still uses Command Line Interface for important operations.
** Cons: User may take a significant amount of time to type out the image path.
* **Alternative 2 :** Drag and drop an image onto the GUI.
** Pros: More intuitive for the user as there is no need to deal with image paths.
** Cons: Deviates from the Command Line Interface.
...
** Pros: No needs for imports. Also, it is possible to declare new mappings on the run and use it directly in the `_CommandBox_`.
...
** Cons: No default handler for kbd:[Enter]. Furthermore, the <<CSS,CSS>> is particularly hard to handle due to the way `_TextArea_` and its body function separately.
...
The command is implemented in `_ParseCommand_`. When user submits the command to the *`UI`* component, it passes on the command to the *`Logic`* component, which posts a `_WebParseRequestEvent_` to the `_EventsCenter_`. Refer to <<fig-ParseCommandSD>>.
...
The event is then propagated through the `_EventsCenter_` to the `*UI*`. The `*UI*` will call `handleWebParseRequestEvent()`. Refer to <<fig-ParseCommandEventSD>>.
...
Refer to <<fig-WebParserCD>> for the interactions between the three classes mentioned above. +
{nbsp} +
...
** Cons: DOM queries are very expensive. Furthermore, IntelliJ does not support variable debugging on w3c.dom Document, which, as a result, makes parsing for the correct elements a game of lottery.
...
** Cons: This approach does not fully test for the correctness of `WebParserHandler` constructor. And usually the tests fail even though they should pass.
* **Alternative 2:** Do a correct test by using a long waiting time.
** Pros: This approach works.
** Cons: But it makes testing a lot slower due to the long waiting time.
// end::web-parse[]
...
The command is implemented in `_SearchCommand_`. When user submits the command to the *`UI`* component, it passes on the command to the *`Logic`* component, which creates a `_WikiaQueryHandler_` and posts a `_InternetSearchRequestEvent_` to the `_EventsCenter_`. Refer to <<fig-InternetSearchSD>>.
...
The event is then propagated through the `_EventsCenter_` to the `*UI*`. The `*UI*` will call `handleInternetSearchRequestEvent()` to search for the recipe. Refer to <<fig-InternetSearchEventSD>>.
...
* **Alternative 2:** Do not use `_WikiaQueryHandler_` and let `_BrowserPanel_` directly load the query URL.
...
`UndoRedoStack` only deals with `UndoableCommands`. Commands that cannot be undone will inherit from `_Command_` instead. The following diagram shows the inheritance diagram for commands:
...
The user executes a new `_UndoableCommand_`, *`delete 5`*, to delete the 5th recipe in the recipe book. The current state of the recipe book is saved before the *`delete 5`* command executes. The *`delete 5`* command will then be pushed onto the `undoStack` (the current state is saved together with the command).
...
Commands that are not undoable are not added into the `undoStack`. For example, *`list`*, which inherits from `_Command_` rather than `UndoableCommand`, will not be added after execution:
...
** Cons: Classes that inherit from `_UndoableCommand_` must remember to call `super.execute()`, or lose the ability to undo/redo.
