...
/**
 * Provides a handle for {@code SearchResultsPanel} containing the list of {@code BookCard}.
 */
public class SearchResultsPanelHandle extends NodeHandle<ListView<BookCard>> {
    public static final String SEARCH_RESULTS_LIST_VIEW_ID = "#searchResultsListView";

    private Optional<BookCard> lastRememberedSelectedBookCard;

    public SearchResultsPanelHandle(ListView<BookCard> searchResultsPanelNode) {
        super(searchResultsPanelNode);
    }
    ...
    public BookCardHandle getHandleToSelectedCard() {
        ...
        if (bookList.size() != 1) {
            throw new AssertionError("Search results list size expected 1.");
        }
    ...
    public BookCardHandle getBookCardHandle(Book book) {
        Optional<BookCardHandle> handle = getRootNode().getItems().stream()
                ...
                .findFirst();
        return handle.orElseThrow(() -> new IllegalArgumentException("Book does not exist: " + book.toString() + "."));
    }
