...
The given dictionary and the constant `MAX_SUGGESTIONS` can be found in `AutoCompletionUtil.java`. All operations with input text such as finding the last word of user's input are supported by `TextInputProcessorUtil.java`.

To calculate position to display pop-up menu, height and width of input text (in pixels) are computed and then used to determine display position in the main window. Height and width of input text are calculated in `TextInputProcessorUtil.java`.
...
----
Because the file is being uploaded to the user's Dropbox account, their Dropbox account access token is needed and is stored in the variable _ACCESS_TOKEN_. This token is then used together
with `String` _XmlExtensionFilename_, which was obtained from `UploadCommandParser`, in the `upload()` method as shown below:
...
----
The _ACCESS_TOKEN_ is currently hardcoded in, as shown below, but will be improved upon soon.
