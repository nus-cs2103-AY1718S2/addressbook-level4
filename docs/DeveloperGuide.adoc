...
=== Prerequisites
Given below are the required prerequisites for developing contactHeRo.
...
image::DetailsPanelClassDiagram.png[width="600"]
...
image::ContactDetailsDisplayWireFrame.png[width="600"]
...
==== Current implementation
`ProfilePicture` is a `Person` 's attribute. It stores the path of the input profile picture and generates a similar profile picture which will be stored in the app.
The ProfilePicture constructor is called when `AddCommand` or `EditCommand` is executed. In `AddCommand`, it is an optional attribute. Whenever there is no valid profile picture for `ProfilePicture`,
`ProfilePicture` takes the default `ProfilePicture`.
`ProfilePicture`.getImage() returns an image for the `ImageView` of UI.

==== Design consideration
===== Aspect: How to link with `ProfilePicture`
* **Alternative 1 (current choice):** Takes in the image path then copy the image to the ProfilePictureFolder.
** Pros: We still have profile picture if the original profile picture is lost.
** Cons: Memory consumption.
* **Alternative 2:** Store the path and retrieve the image from the path when necessary.
...
==== Current implementation
The `EmailCommand` class will allow user to open an email interface to send email to the chosen contact. `EmailCommand` extends the `Command` class. It takes in an index through the `EmailCommandParser`,
similar to the implementation of `DeleteCommand` and `DeleteCommandParser`.

The sending of email will be implemented using Gmail API. First, `GoogleAuthorizationCodeFlow` is used to build and request for authorization from user.
Then it saves the user authorization into `Credential`, which is saved into a local file so that they do not need to repeat the authorization step.
...
*Scenario:* You are in charge of `model`. One day, the `logic`-in-charge approaches you for help. He wants to implement a command such that the user is able to remove a particular skill from everyone in contactHeRo, but the model API does not support such a functionality at the moment. Your job is to implement an API method, so that your teammate can use your API to implement his command.

[TIP]
Do take a look at <<Design-Model>> before attempting to modify the `Model` component.

. Add a `removeTag(Tag)` method. The specified skill will be removed from everyone in contactHeRo.
+
****
* Hints
...
**  Find out which of the existing API methods in  link:{repoURL}/src/main/java/seedu/address/model/AddressBook.java[`AddressBook`] and link:{repoURL}/src/main/java/seedu/address/model/person/Person.java[`Person`] classes can be used to implement the skill removal logic. link:{repoURL}/src/main/java/seedu/address/model/AddressBook.java[`AddressBook`] allows you to update a person, and link:{repoURL}/src/main/java/seedu/address/model/person/Person.java[`Person`] allows you to update the skills.
* Solution
** Implement a `removeTag(Tag)` method in link:{repoURL}/src/main/java/seedu/address/model/AddressBook.java[`AddressBook`]. Loop through each person, and remove the `skill` from each person.
** Add a new API method `deleteTag(Tag)` in link:{repoURL}/src/main/java/seedu/address/model/ModelManager.java[`ModelManager`]. Your link:{repoURL}/src/main/java/seedu/address/model/ModelManager.java[`ModelManager`] should call `AddressBook#removeTag(Tag)`.
** Add new tests for each of the new public methods that you have added.
** See this https://github.com/se-edu/addressbook-level4/pull/790[PR] for the full solution.
*** The current codebase has a flaw in skills management. Tags no longer in use by anyone may still exist on the link:{repoURL}/src/main/java/seedu/address/model/AddressBook.java[`AddressBook`]. This may cause some tests to fail. See issue  https://github.com/se-edu/addressbook-level4/issues/753[`#753`] for more information about this flaw.
...
. Use different colors for different skills inside person cards. For example, `friends` skills can be all in brown, and `colleagues` skills can be all in yellow.
+
**Before**
+
image::getting-started-ui-skill-before.png[width="300"]
+
**After**
+
image::getting-started-ui-skill-after.png[width="300"]
+
****
* Hints
** The skill labels are created inside link:{repoURL}/src/main/java/seedu/address/ui/PersonCard.java[the `PersonCard` constructor] (`new Label(skill.skillName)`). https://docs.oracle.com/javase/8/javafx/api/javafx/scene/control/Label.html[JavaFX's `Label` class] allows you to modify the style of each Label, such as changing its color.
** Use the .css attribute `-fx-background-color` to add a color.
** You may wish to modify link:{repoURL}/src/main/resources/view/DarkTheme.css[`DarkTheme.css`] to include some pre-defined colors using css, especially if you have experience with web-based css.
* Solution
** You can modify the existing test methods for `PersonCard` 's to include testing the skill's color as well.
 ...
 ** Major Feature: A recruiter needs the list of the job openings in the company so he knows what are the current job openings and what skillsets to look for in people. This way, the user can better search for potential employees.
...
* Minor Feature: Find contact by skill
** User can find contact by skill using keyword of the skill.
...
|`* * *` |user |add skill |categorize people

|`* * *` |user |find people by skill | find people with similar skill

|`* * *` |user |view LinkedIn profile | stay updated about the person's professional advancement and achievement

|`* * *` |user |add jobs | find people with similar skill
