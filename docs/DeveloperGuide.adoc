...
image::sortedlist.png[width="790"]
==== Design Considerations
...
===== Aspect: Identify top clients
 * **Alternative 1(current choice):** Sort by commission
 ** Pros: Easy to implement
 ** Cons: Difficult to view ranking of individual insurance plan (e.g top Health plan client)
 * **Alternative 2:** Colour code top few client
 ** Pros: Can sort by insurance plan to know if client belong to that category
