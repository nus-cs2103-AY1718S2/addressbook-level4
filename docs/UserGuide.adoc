...
==== Editing the medical records for a patient : record [since v1.3]

Edits the medical records for a patient specified by the index number used in the last patient listing. +
Format: record INDEX in/[RECORD INDEX] d/[DATE] s/[SYMPTOMS] i/[ILLNESS] t/[TREATMENT] r/[REMARKS]

**
* Selects the person at the specified `INDEX` and edits the medical records at the specified `RECORDINDEX` for that patient.
* The index refers to the index number shown in the most recent listing.
* The index *must be a positive integer* `1, 2, 3, ...`
**
...
* list +
record 1 in/2 d/19 March 2018 s/Runny nose, Headache i/Flu t/Zyrtec +
Edits the second record for the first person to Date:19 March 2018 Symptoms:Runny nose, Headache
