...
import static seedu.flashy.logic.commands.SelectCommand.MESSAGE_SELECT_TAG_SUCCESS;
...
public class SelectCommandSystemTest extends CardBankSystemTest {
    ...
    public void select() {
         ...
         * -> selected
         */
        String command = "   " + SelectCommand.COMMAND_WORD + " " + INDEX_FIRST_TAG.getOneBased() + "   ";
        assertCommandSuccess(command, INDEX_FIRST_TAG);

        /* Case: select the last card in the tag list -> selected */
        Index tagCount = Index.fromOneBased(getTypicalTags().size());
        command = SelectCommand.COMMAND_WORD + " " + tagCount.getOneBased();
        assertCommandSuccess(command, tagCount);
        ...
        Index middleIndex = Index.fromOneBased(tagCount.getOneBased() / 2);
        ...
        /* Case: filtered tag list, select index within bounds of flashy book but out of bounds of tag list
         * -> rejected
         */
        ...
        /* Case: filtered tag list, select index within bounds of flashy book and tag list -> selected */
        ...
        /* Case: select from empty flashy book -> rejected */
        clearCardBank();
    ...
    private void assertCommandSuccess(String command, Index expectedSelectedCardIndex) {
        ...
        String expectedResultMessage = String.format(
                MESSAGE_SELECT_TAG_SUCCESS, expectedSelectedCardIndex.getOneBased());
        int preExecutionSelectedCardIndex = getTagListPanel().getSelectedCardIndex();
