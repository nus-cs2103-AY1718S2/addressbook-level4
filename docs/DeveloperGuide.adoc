...
After you do the steps above, the Settings window should appear as shown in the picure below.

image::Import Setting.png[Width="400"]
...
[[Design-Architecture]]
=== Architecture
_Figure 1_ below illustrates the high-level design of the application.
...
For example, the `Logic` component defines its API in the `Logic.java` interface and exposes its functionality using the `LogicManager.java` class. _Figure 2_ below is an overview of the Logic component.
...
_Figure 3_ below shows how the components interact in a scenario where the user issues the command `delete 1`.
...
_Figure 6_ below shows the architecture diagram of the Logic component.
[[fig-LogicClassDiagram]]
.Structure of the Logic Component
image::LogicClassDiagram.png[width="800"]

Finer details concerning `XYZCommand` and `Command` in <<fig-LogicClassDiagram>> in _Figure 7_ below.
...
_Figure 9_ below shows the architecture diagram of the Logic component.
...
==== Displaying Task
A single `Task` is displayed using a `TaskListCard`. The list of all the `Task` added by the user
is then displayed using the `TaskListPanel`. Below is an example of a `TaskListCard`.
...
==== Displaying Event
A single `Event` is displayed using a `EventListCard` . The list of all the `Event` added by the user
is then displayed using the `EventListPanel`. Below is an example of a `EventListCard`.

.EventListCard example
image::EventCard.png[width='550']
Operations that manipulate the GUI with respect to `Task` class includes `ListCommand`.
{empty} +
{empty} +
...
Storing of `Task` and `Event` data is managed by `StorageManager` within the `Storage` component. `Task`
and `Event` is converted to `XmlAdaptedTask` and `XmlAdaptedEvent` respectively to be stored in a .xml file.
{empty} +
{empty} +

==== Design Consideration
The application deals with 2 main classes: Task and Event. Hence, the data is stored, there is a need for
