...
public class Organizer implements ReadOnlyOrganizer {
    ...
    private Task syncWithMasterTagList(Task task) {
        ...
        return new Task(
                task.getName(), task.getPriority(), task.getDeadline(), task.getAddress(), correctTagReferences);
    }
    ...
    private void removeTagFromTask(Tag tag, Task task) throws TaskNotFoundException {
        ...
        Task newTask =
                new Task(task.getName(), task.getPriority(), task.getDeadline(), task.getAddress(), newTags);
