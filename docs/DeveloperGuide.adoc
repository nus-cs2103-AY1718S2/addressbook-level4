...
image::DeleteBeforeSequenceDiagram.png[width="900"]
...
* **Alternative 2:** Instead of extending `UndoableCommand` it would be possible to extend `AddCommand` class, and overwrite it to add a while loop to loop over the contacts imported frmo CSV and add them to the model
...
* **Alternative 2:** Because we know there can only be 1 valid argument, we could check for validity and correctness in the `ImportContactsCommand` class and not impliment a parser
...
date who matches all tags specified by the user
...
*** Implementing `removeappointment` command to support the removal of appointments
...
** Upgrades the command box to show hints dynamically
...
|`* * *` |user |delete old, inactive contacts in batches |clear up space for new contacts
...
|`* * *` |user |manage appointment dates and details in a calendar |keep track of them systematically

|`* * *` |user |send email using customised templates |easily follow up with clients

|`* * *` |user |export the contacts to CSV format files |further make use of them

|`* * *` |user |import contacts from CSV format files |speed up the process of adding new contacts
...
|`* * *` |user |set reminders |be reminded about appointments and events

|`* * *` |user |include client photos in their contact details |recognise them more easily

|`* * *` |user |enjoy autocomplete when typing commands |save time

|`* * *` |user |use truncated commands |remove entries that I no longer need

|`* * *` |user |customise the colour of the tags |view the different tags more clearly

|`* * *` |user |replace an existing tag with a new tag |update tags effectively

|`* * *` |user |delete a tag from everyone in the address book |save time

|`* *` |user |prioritise clients |single out the more profitable clients

|`* *` |user |email clients from the address book |make arrangements with them
...
|`*` |user |link to the clients' social media page |find out more about them
