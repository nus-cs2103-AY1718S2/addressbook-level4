...
`ToDo`,`Content`,`Status` and `UniqueToDoList` class reside inside `AddressBook`. The following is the class diagram showing the relationship between `ToDo`,`Content`, `Status`, `UniqueToDoList` and `AddressBook`:
...
Suppose that the user has just launched the application. The `UniqueToDoList` in the address book will be empty if no to-do has been added previously.
...
The new to-do with content and status is added to the `UniqueToDoList` inside `AddressBook` and the current state of the address book is saved.
...
** Cons: Extensive refactoring of existing tests is requirred.
// end::addToDo[]
...
The check/uncheck to-dos mechanism is facilitated by `CheckToDoCommand` and `UnCheckToDoCommand`, which resides inside `Logic` component. It supports modifying Status object within ToDo object. CheckToDoCommand and UnCheckToDoCommand inherit from `UndoableCommand`.

Hence, CheckToDoCommand and UnCheckToDoCommand can be undone using `UndoRedoStack`.

Each `ToDo` object has a `Status` object, representing the status of the to-do.
...
The existing to-do is replaced by the new ToDo in the `UniqueToDoList` and the current state of the address book is saved.
The following sequence diagram shows how the check operation works:

image::CheckToDoSequenceDiagram.png[width="800"]

The following sequence diagram shows how the uncheck operation works:
...
The edit to-dos mechanism is facilitated by `EditToDoCommand`, which resides inside `Logic` component. It supports modifying Content object within ToDo object. EditToDoCommand inherits from `UndoableCommand`.
...
The existing to-do is replaced by the new to-do in the `UniqueToDoList` within `AddressBook` and the current state of the address book is saved.
...
The delete to-dos mechanism is facilitated by `DeleteToDoCommand`, which resides inside `Logic` component. It supports deleting ToDo objects. DeleteToDoCommand inherits from `UndoableCommand`.
...
. Other incorrect deleteToDo commands to try: `deleteToDo Swim`, `deleteToDo x` (where `x` is larger than the list size) _{give more}_ +
   Expected: Similar to previous.
...
. Other incorrect check commands to try: `check Swim`, `check x` (where `x` is larger than the list size) _{give more}_ +
   Expected: Similar to previous.
...
. Other incorrect uncheck commands to try: `uncheck Swim`, `uncheck x` (where `x` is larger than the list size) _{give more}_ +
   Expected: Similar to previous.
...
. Other incorrect editToDo commands to try: `editToDo c/Swim`, `editToDo x c/Swim` (where `x` is larger than the list size) _{give more}_ +
   Expected: Similar to previous.
