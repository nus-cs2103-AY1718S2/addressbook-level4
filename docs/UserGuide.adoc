...
ifdef::env-github[]
Please click https://recirecipe.netlify.com/userguide[here] to view the User Guide with full formatting.
endif::[]

== Introduction

// tag::applicationDescription[]
*<<ReciRecipe,ReciRecipé>>* is *a recipe management desktop app with a built-in web browser*. ReciRecipé is *optimized for those who prefer to work with a Command Line Interface* (<<CLI,CLI>>) while still having the benefits of a Graphical User Interface (<<GUI, GUI>>). +
...
* Press kbd:[Ctrl]+kbd:[Tab]/kbd:[Tab] to quickly move the caret to previous/next field.
// end::app-navigation[]
...
`parse` _is intended to work with recipe pages only. ReciRecipé might or might not parse other types of pages. Restricting `parse` to work on only recipe pages will be considred for v2.0._
...
* kbd:[F1] and kbd:[F2] don't work when the in-app browser is clicked. +
_This is a known problem with current version of Java. We have been looking into this problem and the issue might be resolved soon._
