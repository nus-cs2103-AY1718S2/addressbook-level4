...
. Click on the `Imports` tab to set the following options:
...
See <<UsingTravis#, UsingTravis.adoc>> to learn how to set up Travis to perform Continuous Integration (CI) for your fork.
...
.  Set the destination to `Save as PDF`, then click `Save` to save a copy of the file in PDF format. For best results, use the settings indicated in the screenshot (Fig. 11) below.
...
Thanks to the https://github.com/TestFX/TestFX[TestFX] library we use, our GUI tests can be run in _headless_ mode. In headless mode, GUI tests do not show up on the screen. That means the developer can do other things on their computer while the tests are running.
...
A project often depends on third-party libraries. For example, Address Book depends on the http://wiki.fasterxml.com/JacksonHome[Jackson library] for XML parsing. Gradle can manage these _dependencies_ for you by downloading the dependencies automatically, instead of having to: +
