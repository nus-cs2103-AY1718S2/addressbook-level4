...
You may wish to refer to the https://github.com/CS2103JAN2018-F09-B1/main/blob/master/docs/DeveloperGuide.adoc#setting-up[set up] portion
to set up the Your TA project file.

Read through the rest of the guide to familiarise yourself with how Your TA components function together.
...
Upon setting up the app, the Login component takes in two inputs from the user: `Username` and `Password`, then stores the user's login credentials into a `.xml` file.
...
[NOTE]
It may be difficult to edit the `.fxml` file directly. *Javafx* "Scene Builder" is recommended to be used to edit or creat new `.fxml` files.

.Layout of the UI
image::UiDisplay.png[width="800"]

As seen from the layout in figure 6, from top to bottom: +

* MenuBar, `File` and `Help` are dropdown menus that perform their usual functions. As seen in Figure 7 below.

.Menu bar of the UI
image::MenuBar.png[width="800"]

* Down to the left, is the tab consisting list of people in the address book. As seen in Figure 9 below.
** It implements the link:{repoURL}/src/main/java/seedu/address/ui/PersonListPanel.java[`PersonListPanel`] class whose layout is specified in link:{repoURL}/src/main/resources/view/PersonListPanel.fxml[`PersonListPanel.fxml`].
** The individual person card implements the link:{repoURL}/src/main/java/seedu/address/ui/PersonCard.java[`PersonCard`] class whose layout is specified in link:{repoURL}/src/main/resources/view/PersonListCard.fxml[`PersonCard.fxml`].

.Person list of the UI
image::PersonList.png[width="350"]

* The tab to the right of Person List is a list of todo tasks in the addressbook. As seen in Figure 10 below. (Not yet completed)
** It implements the link:{repoURL}/src/main/java/seedu/address/ui/TodoListPanel.java[`TodoListPanel`] class whose layout is specified in link:{repoURL}/src/main/resources/view/TodoListPanel.fxml[`TodoListPanel.fxml`].
** The individual todo card implements the link:{repoURL}/src/main/java/seedu/address/ui/TodoCard.java[`PersonCard`] class whose layout is specified in link:{repoURL}/src/main/resources/view/TodoListCard.fxml[`PersonCard.fxml`].

.Todo list of the UI
image::TodoList.png[width="350"]

* To the right, is the calendar of the address book, in V2.0 task can be scheduled into this calendar. As seen in Figure 11 below.
** It implements the link:{repoURL}/src/main/java/seedu/address/ui/CalendarView.java[`CalendarView`] class whose layout is specified in link:{repoURL}/src/main/resources/view/CalendarView.fxml[`CalendarView.fxml`].

.Calendar of the UI
image::Calendar.png[width="800"]

* At the bottom are the command line and results panel. This command line is where the user would type in the commands for the application to execute, and the results of the command will be displayed below in the result panel. as seen in Figure 10 below.
** The command line implements the link:{repoURL}/src/main/java/seedu/address/ui/CommandBox.java[`CommandBox`] class whose layout is specified in link:{repoURL}/src/main/resources/view/CommandBox.fxml[`CommandBox.fxml`].
** The results panel implements the link:{repoURL}/src/main/java/seedu/address/ui/ResultDisplay.java[`ResultDisplay`] class whose layout is specified in link:{repoURL}/src/main/resources/view/ResultDisplay.fxml[`ResultDisplay.fxml`].

.Command line of the UI
image::CommandLine.png[width="800"]
...
* Calendar added as of v1.3.
* Calendar responds to events raised by the App in V2.0.
...
This feature allows the user to call their desktop mail application from the application itself, with the to: field already populated.
...
The user will enter a command `email 1` to the application. The application will then rely on `AddressBookparser` and `EmailCommandParser` to
...
The application will take in the argument through `dp/ [PATH TO IMAGE]` and then use `ImageIO` to copy the file.
...
* Support to delete photo (Coming in v1.3)
* Support for undo/redo (Coming in v1.4)
* Support for photo frames (Coming in v1.5)
