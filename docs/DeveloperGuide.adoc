...
The following sequence diagram will show how input arguments accompanying the `Find` command are parsed:
...
The following sequence diagram expands on the Parse Sequence block in the previous diagram.

.Sequence diagram describing the sequence of parser operations
image::ParserUtilSequence.png[]
...
===== Argument Tokenizing
We will illustrate the flow of the tokenizing an example input:
```
...
Below is a sequence diagram describing the behaviour of `ArgumentTokenizer` on the input:
...
Further details on how this is performed is delved into in the later <<Concept of parsing,section>>.
...
Further details on how this is performed is delved into at the later <<Generating the Conditions,section>>.
...
4. The tags appear on all the coins in the CoinBook.
5. User then operates on all coins that were tagged. e.g. buy tag 50
6. CoinBook shows the changes to the User.
