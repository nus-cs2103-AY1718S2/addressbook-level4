...
<<<<<<< HEAD
=== Locating persons by tags: `tag`

Finds person whose tags contain any of the given keywords.
...
* Persons matching at least one keyword will be returned (i.e. `OR` search). e.g. `friends family` will return persons with either `friends` OR `family` as their tags.
****

Examples:

* `tag friends` +
Returns persons with the tag `friends`
* `tag friends family` +
Returns any person with the tag `friends` or `family`
