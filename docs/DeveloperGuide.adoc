...
defined by the user. The class diagram for the Template is as follows:

image::TemplateClassDiagram.png[width="900"]
...
with functionality to manage the filtered list of `Template`. The extension is kept as minimised as possible in light of
the *YAGNI* Principle.
...
The `AddTemplateCommand` inherits from the `UndoableCommand` to allow user to undo the addition of an `Template`. The
implementation of the `AddTemplateCommand` is similar to that of the `AddCommand`.
...
with its associated `DeleteTemplateCommandParser` class. This function supports the deletion of an `Template` from the address book.
The `DeleteTemplateCommand` inherits from the `UndoableCommand` to allow user to undo the deletion of an `Template`. The
...
image::AppointmentClassDiagram.png[width="900"]
