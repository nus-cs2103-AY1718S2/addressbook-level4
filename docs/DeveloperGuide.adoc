...
relation between a flashcard and a tag. The software requires that:
...
`Model`. The model exposes two methods:

1. `addEdge(Card card, Tag tag)`: adds an edge between `card` and
`tag`.

2. `removeEdge(Card card, Tag tag)`: removes the edge between `card` and
`tag`.
