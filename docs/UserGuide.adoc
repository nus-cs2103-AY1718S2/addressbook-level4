...
* You will be shown a list of suggested NRICs when when you need to reference an existing contact in Medeina e.g. adding a pet patient / appointment, editing a pet patient to change ownership.
...
If you want to list out all Contacts and PetPatients stored in Medeina for a better overview, enter `list` or `ls` in the command box.
...
When you realise you've entered a command by mistake (e.g. accidentally deleted a petPatient) and want to undo that action? Simply enter `undo` or `ud` in the command box.
