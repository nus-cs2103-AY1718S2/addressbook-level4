...
//        /* Case: group with valid name -> successful */
        String command = ViewGroupCommand.COMMAND_WORD + WHITESPACE + GROUP_NAME;
        String expectedResultMessage = String.format(ViewGroupCommand.MESSAGE_SUCCESS, GROUP_NAME);
        assertCommandSuccess(command, expectedResultMessage);

        /* Case: group name is missing -> rejected */
//        command = ViewGroupCommand.COMMAND_WORD;
//        expectedResultMessage = Inva;
//        assertCommandFailure(command, expectedResultMessage);
    }
