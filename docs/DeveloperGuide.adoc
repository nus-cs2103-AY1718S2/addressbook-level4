...
=== Autosorting in UniqueActivityList
...
----
`UniqueActivityList` contains a list of unique activities which is either a `Task` or an `Event` and they is stored in `internalList`. Each time an `Activity` is added to or edited in `UniqueActivityList`, `internalList` will be sorted according to its `DateTime`. Note that for events, its `DateTime` refers to the event's starting time.

=== Task
Task refer to a piece of work to be done that can be added by a user of the application. After being added, tasks can be edited and deleted. When a task is not completed within the deadline
...
Operations that manipulate a `task` class object can been done using `TaskCommand`, `CompleteCommand`, `RemoveCommand`, `EditCommand` commands.
These commands are described in more detail below.

Operations that manipulate the GUI with respect to `Task` class includes
`ListCommand`, `OverdueCommand`.

===== Adding tasks
Tasks can be added through the use of the `TaskCommand`.

The following is a snippet shows how the `executeUndoableCommand()` of `TaskCommand` updates `UniqueActivityList` residing in model of CLIndar by adding `toAdd`. Note that `toAdd` will not be added if a duplicate task has been found or the `Name` or `DateTime` field is left empty.
...
===== Completing Tasks
Tasks can be completed through the use of the `CompleteCommand` by specifying the task's index reflected in the GUI.
...
===== Removing Tasks
Tasks can be removed through the use of the `RemoveCommand` by specifying the index of the task reflected in the GUI. `UniqueActivityList` residing in the `model` will be updated by having `activityToDelete` deleted from it.

The following snippet shows the `executeUndoableCommand()` of the `RemoveCommand` class. If the index of an activity is not found, an `ActivityNotFoundException` will be thrown.
...
----
If the `RemoveCommand` is successful, the relavant `Task` or `Event` with the specified index will be removed in the GUI.
...
A user can view his overdue tasks through the use of the `OverdueCommand`. Note that overdue only lists down overdue tasks; overdue events are marked as
completed automatically.

We will be making use of the LocalDateTime class in Java. The class has a `now` method, which allows us to obtain the current system clock.
...
As we modified the application to `CLIndar` we need to store information of activities instead.

The original application `Address Book` store information in the `Person` class.

Since we now need to store information of events and tasks in the memory, the `Person` class needs to modified to store the record.

The `UniquePersonList` needs to be modified to store list of `Task` and `Activity`.
{empty} +
{empty} +
...
Both classes were iherited  from the `Activity` Class.
** Pros:
No need to make major change to the current interface (except some renaming).
** Cons:
It would be a hassle to retrieve information about information stored in other class
Since `Task` and `Event` contains extra information and hence have extra excess method.
Need to cast the object to retrieve the information.
...
While either implementations has its pros and cons.
For the purpose of having incremental modification. We choose Alternative 1.
As such we can modified the implementation without modification to Interface, hence other component would be unaffected.
This would allow us to have a working release every week.
