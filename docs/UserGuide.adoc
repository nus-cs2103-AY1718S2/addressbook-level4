...
Employees Tracker (E.T.) is a *Human Resource Manager* application for managers who *prefer to use a desktop app for managing his/her subordinates*.
E.T. is *optimized for those who prefer to work with a Command Line Interface* (CLI) while still having the benefits of a Graphical User Interface (GUI). If you can type fast, E.T. can get your employees management tasks done faster than traditional GUI apps.
...
_Figure 2_ shows the 6 major components of the user interface of Employees Tracker:

. Command Box:where you type in commands
. Result Display Box: where Employees Tracker shows text result to you
...
. Notification cards: where Employees Tracker reminds you about ended events
. Notification center: where hidden notification cards are stored
...
* Error messages, if the command you entered does not exists or does not match the pre-defined format.
* Hints about the format and usage of the command, if you entered a command in the wrong format.
* Statuses of the Employee Tracker, if your command is processed successfully.

[NOTE]
If the result display box shows text in red, it means that you have entered an non-existing command or a command that does not follow its pre-defined format.
...
There will be 2 notifications for each event, which are:

* notification on the day of expiry
* notification at the time of expiry.

For example, suppose you have assigned a task to Alex which will due on 27th May 2018 4:00 pm. When you open ET for the first time at 8:00 am on 27th May 2018, you will be notified that Alex has a task that will due on *today* . Later at 4:00 pm of 27th May 2018, you will receive another notification, informing you that Alex should have finished the task by *now* .

===== Notification cards and the Notification Center

Notifications are shown in notification cards in ET. Each notification card contains 3 information about the expiring event: title, name of employee in-charge, time of expiry. +

Notification cards will pop into the User Interface of ET from the bottom right corner, and waits 5 seconds before exitting the screen. However, the notification cards aren't dismissed. You can still revisit them in the notification center. +

In the notification center, you can either dismiss a notification, email to remind, and/or whatsapp to remind.
...
=== Dismissing a notification : `dismiss`

Dismisses thea notification identified by the index number used in notification cards. +
Format: `dismiss INDEX`

=== Toggling the notification panel : `noti`
