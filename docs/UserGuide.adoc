...
Adds a person to the instruction book +
...
Shows a list of all persons in the instruction book. +
Format: `list`

=== Editing a person : `edit`

Edits an existing person in the instruction book. +
...
Edits the phone number and ingredient instruction of the 1st person to be `91234567` and `johndoe@example.com` respectively.
...
Deletes the specified person from the instruction book. +
Format: `delete INDEX`
...
* `list` +
`delete 2` +
Deletes the 2nd person in the instruction book.
* `find Betsy` +
`delete 1` +
...
* `list` +
`select 2` +
Selects the 2nd person in the instruction book.
* `find Betsy` +
`select 1` +
