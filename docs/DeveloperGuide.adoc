...
The following diagram illustrates a sequence of method calls to initialize the `Autocomplete` class and get autocomplete suggestions for command words in a drop down menu.
...
You may have noticed that in sequence diagram above (Figure 12), `Autocomplete` is a singleton class that is initialized only once, and there is only 1 instance of `CommandBox` in the application. You may be wondering "are the 3 set attribute methods in the `Logic` component only called once?". If you are currently unsure, imagine the user is constantly adding new contacts to Medeina. However, `Autocomplete` uses only data from when Medeina was started up. +
...
** Pros: Easy to implement.
** Cons: Running for-loops in the `Logic` component for every character the user types, is resource-intensive.
