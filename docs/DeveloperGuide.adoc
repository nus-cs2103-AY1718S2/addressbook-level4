...
`UniqueMilestoneList` and `UniqueTaskList` are associated to `ObservableList<Milestone>` and `ObservableList<Task>` respectively that can be 'observed'.
...
[source, java]
----
    private Student createEditedStudent(Student studentToEdit, Milestone newMilestone) throws DuplicateMilestoneException {
