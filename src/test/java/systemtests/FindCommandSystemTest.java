...
public class FindCommandSystemTest extends AddressBookSystemTest {
    ...
    public void find() {
        ...
        // first names of Benson and Daniel are "Meier", first name of ELLE is Meier
        ...
        command = FindCommand.COMMAND_WORD + " " + tags.get(0).name;
        assertCommandSuccess(command, expectedModel);
        assertSelectedCardUnchanged();
        ...
        command = "FiNd Meier";
        assertCommandFailure(command, MESSAGE_UNKNOWN_COMMAND + MESSAGE_DID_YOU_MEAN + FindCommand.COMMAND_WORD);
    }

    /**
     * Executes {@code command} and verifies that the command node displays an empty string, the result display
     * node displays {@code Messages#MESSAGE_PERSONS_LISTED_OVERVIEW} with the number of people in the filtered list,
     * and the model related components equal to {@code expectedModel}.
     * These verifications are done by
     * {@code AddressBookSystemTest#assertApplicationDisplaysExpected(String, String, Model)}.<br>
     * Also verifies that the status bar remains unchanged, and the command node has the default style class, and the
    ...
    /**
     * Executes {@code command} and verifies that the command node displays {@code command}, the result display
     * node displays {@code expectedResultMessage} and the model related components equal to the current model.
     * These verifications are done by
     * {@code AddressBookSystemTest#assertApplicationDisplaysExpected(String, String, Model)}.<br>
     * Also verifies that the browser url, selected card and status bar remain unchanged, and the command node has the
     * error style.
     * @see AddressBookSystemTest#assertApplicationDisplaysExpected(String, String, Model)
     */
