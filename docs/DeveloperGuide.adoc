...
As the user continues to use the program, more commands are added into the `undoStack`, as shown in Figure 13. For example, the user may execute `add n/David ...` to add a new patient.
...
We will restore the address book to the state before the `add` command executed, as shown in Figure 14.
