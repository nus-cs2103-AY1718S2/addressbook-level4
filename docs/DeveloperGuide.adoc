...
The following diagram illustrates the sequence of methods that occurs when adding a new pet patient to Medeina.
...
The follow code shows how dependency between `PetPatient` and `Person` object is verified:
[source,java]
----
...
* **Alternative 1 (current choice):** A single `add` command with options to differentiate amongst adding of different objects.
...
* **Alternative 2:** 3 different add commands e.g. addperson, addpetpatient, addappointment
** Pros: Easy to implement.
** Cons: Will need a new command for syntax (4). More commands for the user to remember. 
