...
.. Prerequisites: User is logged in and has authenticated SLAP via Oauth. See the Calendar Quick Start in the <<UserGuidet#, UserGuide.adoc>>
...
// tag::errorLogTesting[]
.  Displaying the error log within the application
...
.. Test case: `View > Show Error Log` not logged in +
   Expected: Error log WebView opens and the error log is displayed.
