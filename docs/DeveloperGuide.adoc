...
image::RecentCommandListOne.png[width="550"]
...
image::RecentCommandListTwo.png[width="550"]
...
image::RecentCommandListThree.png[width="550"]

The user continues to select books, and the same operations will apply.

image::RecentCommandListFour.png[width="550"]

The `UniqueBookCircularList` is now full. When user performs `select 4`, since the book
...
image::RecentCommandListFive.png[width="550"]
...
. `setKeyCommandParser` creates an `setKeyCommand` with the parameters.
. `setKeyCommand` is preprocessed to check that oldKey is correct.
. If the oldKye is correct, `setKeyCommand` will send a setKey command to LogicManager.
...
* **Customize aliases to use in place of full commands** (major)
** Added `addalias`, `aliases` and `deletealias` commands to allow adding, viewing and deleting of aliases.
** This allows users to reduce the typing needed.
...
** Added attributes rating, status, priority.
