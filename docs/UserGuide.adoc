...
Also, if you minimizes ET to use other software, ET is capable of showing notification cards in your computer's system (e.g. Windows, Mac, Linux, etc.), as shown in _Figure 6_. However, this feature may not work on some computer systems.
...
Shows a list of all employees as shown in _Figure 10_. +
Format: `list`
...
_Figure 11_ shows a pop-up dialog box that will appear after user executes the `review` command. The dialog box will prompts the user to enter the user's identity (as a reviewer) and the review.
