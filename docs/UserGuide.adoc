...
[NOTE]
It is not necessary to include a file path after the command. If no file path is included, `ec` will create and write to a new .csv file, _data/exportToExisting.csv_
...
// tag::scheduling[]
=== Add an appointment : `addappointment` or `aa`
Adds an appointment with a person in the address book that will be shown in the calendar panel. +
...
=== Remove an appointment : `rmappointment` or `ra` `[Coming in v1.4]`
Removes an appointment in the address book. +
Format: `rmappointment n/NAME d/DATE st/STARTTIME et/ENDTIME l/LOCATION` or `ra n/NAME d/DATE st/STARTTIME et/ENDTIME l/LOCATION`

****
* Removes an appointment that matches all the input `NAME`, `DATE`, `STARTTIME`, `ENDTIME` and `LOCATION`.
* The input date *must be in the following format: dd/MM/yyyy*.
* The input time *must be in the following format: HH:mm (24hr format)*.
* The calendar panel will be updated accordingly.
****
...
* `rm n/Betsy d/12/12/2017 st/12:30 et/13:30 l/Gold Park Estate` +
Removes the appointment previously arranged with Betsy on 12/12/2017 from 12:30pm to 1:30pm at Gold Park Estate.

=== Zoom in calendar view : `zoomin` or `zi` `[Coming in v1.5]`
...
=== Zoom out calendar view : `zoomout` or `zo` `[Coming in v1.5]`
...
=== Find an appointment : `findappointment` or `fa` `[Coming in v2.0]`
...
=== Edit an appointment : `editappointment` or `ea` `[Coming in v2.0]`
Edit an appointment in the address book. +
...
* *Remove Appointment* : `rmappointment n/NAME d/DATE st/STARTTIME et/ENDTIME l/LOCATION` +
e.g. `rmappointment n/Betsy d/12/12/2017 st/12:30 et/13:30 l/Gold Park Estate`
* *Zoom In* : `zoomin`
* *Zoom Out* : `zoomout`
* *Find Appointment*: `findappointment KEYWORD...` +
e.g. `findappointment 12/12/2018`
* *Edit Appointment*: `editappointment INDEX [n/NAME] [d/DATE] [st/STARTTIME] [et/ENDTIME] [l/LOCATION]` +
