...
Attach a change listener to command box to detect user input.
[source,java]
----
public CommandBox() {
	// ...logic for CommandBox...
...
How descriptions in the autocomplete suggestions are handled:
[source,java]
----
private void triggerAutocomplete(String newValue) {
	// ...logic for context menu "suggestionBox"...
...
Toggle autocomplete on/off using kbd:[F2] when command box is focused.
[source,java]
----
@FXML
...
** Provide experienced users with the option of toggling autocomplete off in the event that they find the context menu distracting.

// end::autocompleteDG[]

// tag::find[]
=== Finding a Contact or Pet Patient
The find function allows users to find for a specific contact or pet patient through the listed fields.
...
==== Current Limitations
Users are unable to find appointment by type as the CalendarFX runs differently from the other classes.
...
==== Current Implementation
After Medeina implemented CalendarFX, users are able to view appointments at a specified date by clicking on the Calendar. However, as Medeina is a CLI-based application, users should not be required to move their mouse. Thus, there was a need to be able to view appointments of specific dates through the command line.
...
In `execute()`, there is a switch that determines the type of command run based on the type obtained from the parser. The relevant functions to change the Calendar gets called based on the type, and an event gets called to handle the change. A check is done to determine whether there were appointments in the year of the requested date, so that users are not able to randomly jump into years that should not have ever had any appointments (i.e. years before Medeina existed).
...
** Cons: Need to implement functions that help will change views (`listappt` command).
