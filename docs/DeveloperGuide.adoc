...
The implementation logic of the autocomplete feature resides in the UI component. The autocomplete feature is driven by a `ChangeListener` attached to the `commandTextField.textProperty()` in `CommandBox.java`. Once `ChangeListener` registers any changes in the content of `commandTextField`, `triggerAutocomplete()` will be executed. The following diagram illustrates a sequence of method calls to initialize the `Autocomplete` class and get autocomplete suggestions for command words in a drop down menu.

.Sequence diagram for autocompleting Nric (include initialization of Autocomplete class)
image::autocompleteSequenceDiagram.png[width="700"]

`Autocomplete.java` encompasses the logic for parsing user input in the CLI and determining what autocomplete suggestions are to be passed back to `CommandBox.java`. Data required for autocomplete, such as a list of command words, prefixes, options and parameters in Medeina, are all retrieved from the Logic component. 

For example, data for autocompleting Nric and person tags are consolidated this way in the Logic component:
[source,java]
----
...
You may have noticed in the sequence diagram above, `Autocomplete` is a singleton class that is initialized only once, and there is only be 1 instance of `CommandBox` in the application. You may be wondering "are the 3 set attribute methods in the logic component only called once?". If you are not, imagine the user is constantly adding new contacts to Medeina, but `Autocomplete` uses only data from when Medeina was started up. The answer to the question is definitely no. The set attributes methods have to be called whenever there is a change in Medeina's data, otherwise the autocomplete suggestions will not be up to date.

The following code snippet will ensure that `Autocomplete` data is kept updated.
[source,java]
----
...
** Pros: Great improvement in user experience, as Medeina has complex and long command syntax.
...
* **Alternative 1 (current choice):** Data is updated only when AddressBookChangedEvent is raised.
** Pros: Less resource-intensive.
** Cons: Implementation is less straight-forward.
* **Alternative 2:** Data is updated whenever autocomplete is triggered.
** Pros: Easy to implement.
** Cons: Running for-loops in the Logic component for every character the user type is resource-intensive.
...
As of now, the appointment class is yet to be properly developed, thus we are not able to show appointments inside the calendar yet.

Only the UI of CalendarFX is integrated to show calendar view.

==== Future Work

After the appointment class being finished, we'll be able to implement `setAppointment` function in CalendarFX, the user will then be able to see future appointments in either All-day/weekly/monthly/yearly view.
...
|Jacqueline Cheong|
*Make command syntax more user friendly and closer to natural language*

To enhance user experience and usage efficiency by improving the command syntax to be more intuitive and simple to use |

*Add support to change theme*
...
2.  Medeina shows a list of appointments for the current month
3.  User requests to view a specific appointment in the list
4.  Medeina shows the full details of the specified appointment
+
Use case ends.
...
2.  Medeina shows a list of pet patients with the name "Joseph"
3.  User requests to view contact information of a specific pet patient
4.  Medeina shows the pet patient's owner's contact information
5.  User requests to modify the pet patient's owner's contact information
6.  Medeina shows the pet patient's updated contact information
+
Use case ends.
...
+
[none]
** 5a1. Medeina shows the correct command usage to edit pet patient's information.
+
Use case resumes at step 2.
...
4.  Medeina shows the pet patient's medical history
+
Use case ends.
...
2.  Medeina shows a list of owners with "Mavis" as part of their name
3.  User requests to view information of a specific owner
4.  Medeina shows information about the owner
+
Use case ends.
...
4.  Medeina generates a pdf report for the user
+
Use case ends.
...
2.  Medeina shows a list of owners with the name "Mavis"
...
+
[none]
** 3b1. Medeina shows a list of valid tags.
+
Use case resumes at step 2.
