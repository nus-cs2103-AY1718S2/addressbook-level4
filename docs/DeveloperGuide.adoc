...
An `Order` is implemented in the following manner:
[source,java]
----
public class Order {
...
This relationship is shown in _Figure 12_ given below.
...
The code snippet below shows the constructor of this class:
[source,java]
----
public AddOrderCommand extends UndoableCommand {
...
The code snippet below shows the constructor of the class:
...
The code snippet below shows the methods in `EditOrderDescriptor` that implement this functionality for the
`OrderInformation` field of the `order` to be edited:
...
The code snippet below shows the `createEditedOrder()` method that constructs the edited order:

[source,java]
----
public class EditOrderCommand extends UndoableCommand {
    private static Order createEditedOrder(Person orderToEdit, EditOrderDescriptor editOrderDescriptor) {
        assert orderToEdit != null;

        OrderInformation updatedOrderInformation = editOrderDescriptor.getOrderInformation().orElse(orderToEdit.getOrderInformation());

        // ... OrderStatus, Price, and Quantity ...

        DeliveryDate updatedDeliveryDate = editOrderDescriptor.getDeliveryDate().orElse(orderToEdit.getDeliveryDate());

        return new Order(updatedOrderInformation, updatedOrderStatus, updatedPrice, updatedQuantity, updatedDeliveryDate);
    }
}
----

Finally, the `executeUndoableCommand()` method calls the `createEditedOrder()` for getting the edited order and update
...
A single `Order` is displayed using an `OrderListCard` (refer to Figure 13 below). The list of all orders in the address book is then displayed in a list
...
// tag::calendarEvents[]
=== Calendar and events tracking feature "Coming in V1.5"

==== Current Implementation

The event tracking feature is facilitated by `UserEvents`, which resides inside `Model`.
...
The following diagram shows class diagram of `Model` Component related to `UserEvents` and `CalendarEvent`.

image::CalendarClassDiagram.png[width="800"]

In the diagram, `ModelManager` creates an instance of `UserEvents` interface, which provides the `UserEvents` API for `ModelManager` to use.
...
`Logic` Components contains the commands required to manage user's events (e.g. `AddEventCommand`, `deleteEventCommand`) and to change display format of the calendar ui (e.g. `ShowCalendarCommand`).

The sequence diagram below shows how add event operation works:

image::AddEventSD.png[width="800"]
...
public class CalendarEvent {
    ...
    private final List<Person> taggedPersons;
...
* eventTitle: Represents title of event.
...
* startTime: Represents starting time of the event in HHMM format (24-Hour format).
* endTime: Represents ending time of the event in HHMM format (24-Hour format).
* taggedPerson: Represents list of Persons in address book that are tagged to the event.


Events added, removed or modified by users will have their intended results displayed by the `CalendarUi` residing inside `Ui` Component, which displays a calendar showing any future events.
...
****
* Hints
** The tag labels are created inside link:{repoURL}/src/main/java/seedu/address/ui/PersonCard.java[the `PersonCard` constructor] (`new Label(tag.groupName)`). https://docs.oracle.com/javase/8/javafx/api/javafx/scene/control/Label.html[JavaFX's `Label` class] allows you to modify the style of each Label, such as changing its color.
...
*Target user profile*: Salespersons from small companies or home-grown businesses with customers from various social media platforms.
...
3. Ang Jing Zhe
* Major Feature: Add a person panel to the user interface.
** It allows users to see their contacts in a nicer way.
** It is implemented by adding a people's panel code.
** Alternative considerations is to vary the amount of information put up at the panel.
* Minor Feature: Adding colours to tags
** Putting colours to tag so that it looks more user friendly.
** It is implemented by adding colours to java fx.
* How the above features fit into the product scope:
** Major Feature: Users can see the contact more clearly.
** Minor Feature: Colours are more eye-catching.
...
* How the above features fit into the product scope:
** Major Feature: The export and import facility enables users to help salespersons to use the data from ContactSails solely as an addressbook, but gives them the option to more easily integrate it to their current application processes which might allow them to be more efficient in their work.
...
|`* * *` |task-oriented user |implement meetings or deadlines for contacts |keep track of tasks to do for the contacts |2.0
...
|`* *` |user who prefers visual imagery |use a calendar to know if there are any tasks or deadlines |clearer view on how to prioritise my work |2.0
...
|`* *` |lazy user |add contact details in any order |don't need to remember the order to enter the details |2.0
...
|`* *` |user |group contacts using tags |categorize my customers as needed |2.0

|`* *` |user |change the theme and font of the application |personalize my AddressBook |2.0

|`* *` |salesperson |see contact statistics |more informed about my customer demographic |2.0

|`*` |user with many persons in the address book |sort persons by name |locate a person easily |1.0

|`*` |user |sort the list of contacts in alphabetical order |easily find contacts whose names I could not remember |1.0
...
2.  ContactSails prompts user to enter the task details
3.  User enters task description and task deadline
4.  ContactSails adds task to specified contact and shows a success message.
+
Use case ends.
...
+
[none]
** 3a1. ContactSails shows en error message
+
Use case resumes at step 2.
...
1.  User requests to remove a group
2.  ContactSails shows a list of all the groups used in the application
3.  User enters the group they want to remove
4.  ContactSails removes the specific group from all contacts who have this group assigned to them
+
Use case ends.
...
1.  User requests to remove a preference
2.  ContactSails shows a list of all the preferences used in the application
3.  User enters the preference they want to remove
4.  ContactSails removes the specific preference from all contacts who have this group assigned to them
+
Use case ends.
...
[none]
* 3a. User enters invalid preference
+
[none]
** 3a1. ContactSails shows an error message
+
...
2.  ContactSails creates the file and adds all the application data
...
.  As ContactSails is modified from an open-source software, users should be able to download it for free.
.  ContactSails should continue to run the original, intended functions, even when problems occur during execution.
.  In case ContactSails has crashed, changes made to it during those sessions should not be lost.
.  Size of ContactSails should not exceed 100 MB even with many contacts stored in it (maximum number of contacts allowed is 1000)
