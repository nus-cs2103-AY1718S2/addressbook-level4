...
`PetPatient` is dependent on `Person`, and `Appointment` is dependent on both `Person` and `PetPatient`.

* Variations of the same type of command such as:
** **add** (Person), **add**PetPatient, **add**Appointment
...
|`* * *` |user |have an application with a simple and intuitive user interface |maximize workflow efficiency
