...
+
[NOTE]
You need to connect to Internet when using E.T., because some commands need to access Internet.
+
.Authorizing ET
...
****
* Employees are sorted by existing field in alphabetical order, note that rate field will be sorted in descending order.
* The field entered must be one of the following: `name`, `phone`, `email`, `address`, `rate`, `tag`.
* Sorting is case-insensitive.
* Sorted results are stored permanently. If you want to revert to the previous ordering, please refer to <<Undoing previous command : `undo`>>
****
...
* Change of theme will be effective immediately.
* The `THEME` parameter must be one of the following in current version: bright, dark.
****
