...
+
.
.  Run the MainApp scripts and await for the GUI to open
.  A login screen should appear. Create a profile with desired username and password if using for the first time.
...
Sends an email to the current list of customers to remind them when their delivery is coming. +
Format: `email`

This command will email every user listed on the UI screen when the email command is fun

If you want to email a specific person or specific set of people first run the find/filter commands to limit the
displayed list
...
*1.*  add n/John Doe a/321 Clementi Ave p/67340028 e/pigeonscs2103@gmail.com d/2018-03-24
