...
.Export Command Sequence Diagram for Logic Component
...
.Upload Command Sequence Diagram for Logic Component
image::UploadCommandSequenceDiagram.png[width="800"]
...
===== Aspect: How upload command executes

* **Alternative 1 (current choice):** Uploads all `Person` in `filteredPersons`, `Tag`, and `Alias` from StardyTogether.
** Pros: User is able to select which `Person` to be uploaded by using `find` command, user is not able to indicate which `Tag` or `Alias` to be uploaded. User can uploads all `Person` by using `list` command too, storing backups in Google Drive easily.
** Cons: User is not able to select which `Tag` or `Alias` to be uploaded.
* **Alternative 2:** Uploads all `Person`,`Tag`, and `Alias` from StardyTogether.
** Pros: User does not need to indicate which `Person`, `Tag` or `Alias` to be uploaded.
** Cons: User is not able to select which `Person`, `Tag` or `Alias` to be uploaded.

==== Why Download StardyTogether file feature is not implemented?

===== Justification
Downloading a file from Google Drive requires the File ID, as there can be two or more files with the same filename in the user's Google Drive. Hence, the user has to go to his/her Google Drive to get the uploaded StardyTogether File ID.

Since the user is already on his/her Google Drive, he/she may download the file onto his/her computer and perform an `import` function. Steps are not reduced by implementing `download` feature. Therefore, we decided not to implement `download` feature.
// end::datatransfer[]
...
2.  StardyTogether prompts user to input a faculty
3.  User requests venues near this faculty
4.  StardyTogether prints out a list of available venues near the specified faculty
+
Use case ends.
...
+
[none]
** 4a1. StardyTogether shows an error message
