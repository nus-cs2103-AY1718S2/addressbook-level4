= Smart Insurance Collection - Developer Guide
:toc:
:toc-title:
...
==== Current Implementation
The Add appointment feature is facilitated by all the components of the application. It supports adding of entry to the calendar through the command lines.

After the `commandBox` in UI passed the text field input with add_appointment command word to the `LogicManger`, `AddressBookParser` will identify the command word and
call `AddAppointmentCommandParser` which will parse the input text to create a new `AppointmentEntry` object and return a new `AddAppointmentCommand` object. The `LogicManger` will
then execute this command.

The `AddAppointmentCommand` calls `ModelManager` to add an appointment to calendar, `ModelManager` then call `AddressBook' to add appointment and raise a `AddressBookChangedEvent`, finally `AddressBook` calls its `InsuranceCalendar` object to
add an appointment and raise a .

Within `InsuranceCalendar`, the code is as follows:
[source,java]
----
...
updated with the addition of the new appointment. Addition od entry to the `Calendar` object automatically reflects the change on the
`CalendarView` GUI
...
===== Aspect: Implementation of `Add_Appointment command`
...
* **Alternative 1 (current choice):** Ask the user to specify title, start date and end date.
** Pros: Easy to implement.
...
```
ADD n/NAME p/PHONE e/EMAIL a/ADDRESS i/INCOME as/ACTUALSPENDING es/EXPECTEDINCOME age/AGE [t/TAG]...
```
...
* `Function` : Customers in the list may or may not have
"actual spending" entered. Because at the point in time,
the customer has not purchased any policy at all.
Hence it would be convenient to have a way to predict the
likely amount of purchase they would make so that the
insurance agent can prioritize those customers with higher
likelihood to spend more.

* `Implementation` : Each customer's data would be extracted as a row.
The column represents. Each Targeted fields such as income, age and so on.
The model used here is linear regression. That means we assume the purchase made
by the customer is a linear combination of all the targeted fields.
This model is not perfect. However for the first version of this
function, we will just use this model. These data would form a matrix.
Imaging a form of Ax=b where A is a matrix and x is a vector of weights
and b is the expected outcome. The name of the game is to find the
weights that minimises the loss of the objective function A.
The numerical solver used here is Stochastic Gradient Descent.
...
* Minor feature: task pad GUI plus task assignment related functions, allow user to add,remove,modify tasks and reflect these changes on a task pad GUI
...
*Valerian Rey*:
* Minor feature: Policy field to the persons in the AddressBook to specify the insurance contract of a client. It contains multiple classes and enumerations to precisely describe a user's policy (Policy, Price, Date, Month, Issue, Coverage).
...
1.  User click on the calendar to specify the date to add
