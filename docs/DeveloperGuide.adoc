...
* Exposes unmodifiable `ObservableList<Person>` and ObservableList<Task> that can be 'observed' e.g. the UI can be bound to these lists so that the UI automatically updates when the data in the lists change.
...
The application will take in the argument through `dp/ [PATH TO IMAGE]`. If this is empty, the application will default to using the default profile picture
...
=== Task List (Since v1.2)

This feature allows the user to schedule their tasks by entering command inputs in the CLI. Tasks scheduled in this App will be displayed in the Todo List and Calender.

==== Current Implementation

The Task List is a set of task data stored in `addressbook.xml`. The user will manipulate the Task List data in `addressbook.xml` by using commands.
These commands inherit `UndoableCommand`, thus, they support undoing and redoing of commands as well.

The displayed tasks in the `Todo List` and `Calendar View` are unmodifiable view of Task List which is synchronized to addressbook.xml if any changes are made.

===== Adding a task

The `AddressBookParser` will be called by the application and parses arguments to `AddTaskCommandParser`.
This parser will process and validate the arguments as well as
execute `AddTaskCommand` with the task specified by the arguments.

===== Deleting a task

Task index is the only unique indicator that `DeleteTaskCommand` can make use of.
The task index is displayable in `Todo List` which assists the user to decide which task to delete.
`DeleteTaskCommandParser` validates the index parsed in and calls `DeleteTaskCommand` to delete the corresponding task.

===== Editing a task

Editing a task, in fact, is implemented through both deleting a task and adding a task.
The task to be edited is specified by the task index parsed into `EditTaskCommand`.
The rest of arguments specify the detail of task fields the user would like to change.
`EditTaskCommand` encapsulates a new `Task` Object with the modified fields.
The `ModelManager` is then called up to delete the original task and add a new task.
[NOTE]
The edited task will appear to be the last item in the Task List because it is actually a newly added task.

==== Design Considerations

===== Aspect: Edit a task

* **Alternative 1 (Current Choice):** Make a new task after deleting the original task.
** Pros: It is easy to implement as you do not need to implement a individual deadline change mechanism corresponding to the cells of `Calendar View`.
** Cons: The Task List order may not be maintained, the edited task will be added to the last item in the Task List.

* **Alternative 2:** Edit on the fields of original task.
** Pros: The Task List order will be maintained, as it accesses and modifies the item in the Task List.
** Cons: It adds more complex to the feature as it requires a new mechanism to refresh the cells of `Calendar View`.
